...
class StimulusSelectionServiceIntegrationTest {
    ...
    fun realServiceHypertrophyChainIsExecutableButRemainsShadowOnly() = runBlocking {
        ...
        try {
            ...
            val exercise = requireNotNull(db.exerciseDao().findByStableKey(stableKey)) {
                "B101 missing seeded exercise: $stableKey"
            }
            ...
            requireHypertrophyRelation(catalog, stableKey)
    ...
    private fun requireHypertrophyRelation(catalog: CanonicalExercisePhysicalQualityCatalog, stableKey: String) {
        assertTrue(
            "B101 direct hypertrophy relation missing for $stableKey: ${catalog.relations(stableKey)}",
            catalog.relations(stableKey).any {
    ...
    private fun assertHypertrophyServiceResult(
        ...
        assertTrue(h.authorizedPrescription?.sets?.all { it.reps in 7..15 && it.weightKg > 0.0 && it.targetRpeMin == 7.0 } == true)
        assertEquals(StimulusPrescriptionExecutionAuthority.FULLY_ENCODED, h.executionAuthority)
