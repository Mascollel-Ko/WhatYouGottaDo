...
class StimulusSelectionServiceIntegrationTest {
    ...
    fun serviceComparisonUsesRealCanonicalMetadataAndPreservesControlFingerprint() = runBlocking {
        ...
        try {
            ...
            throw AssertionError("B10_DEBUG evaluation=${evaluation.status} reasons=${evaluation.reasonCodes} b7=${evaluation.b7Status} attributions=${comparison.experimentalReadinessAudit?.changeAttributions?.map { it.stableKey to (it.selectionRole to (it.source to it.targetIds)) }} authorizations=${comparison.prescriptionAuthorizationPlan?.authorizations?.map { it.owner to (it.quality to (it.status to it.targetId)) }} materials=${comparison.prescriptionMaterializationAudits.map { it.owner to (it.state to it.reasonCodes) }} resolutions=${comparison.prescriptionRealizationPlan?.resolutions?.map { it.targetId to (it.owner to it.status) }}")
