    ...
    Column(verticalArrangement = Arrangement.spacedBy(14.dp)) {
        fatigueAnalysis.currentState?.let { state ->
            CurrentFatigueStatusCard(
                state = state,
                projectedOfi = fatigueAnalysis.simple.projectedOfiOverlay.lastOrNull()?.value?.roundToInt()
            )
        }
...
private fun PerformanceDetailSectionView(section: PerformanceDetailSection, summary: PerformanceTrendSummary) {
    ...
    val chartSpec = when (section.type) {
        ...
        PerformanceDetailSectionType.BADMINTON ->
            chartBuilder.badmintonPracticeDetail(summary.badmintonPracticeWeeks)
