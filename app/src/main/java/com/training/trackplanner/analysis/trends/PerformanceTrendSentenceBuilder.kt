...
class PerformanceTrendSentenceBuilder {
    fun dashboardSentence(
        ...
        return "근력운동은 $strengthTrend, 배드민턴 연습 훈련량은 $badmintonTrend, 이번 주 누적 부담은 $fatigueTrend 흐름입니다."
            .safeTrendSentence()
    }
    ...
    fun badmintonInterpretation(latest: BadmintonPracticeWeekPoint?): String =
        if (latest == null) {
            "기록이 더 쌓이면 배드민턴 연습 훈련량을 볼 수 있습니다."
        } else {
            "배드민턴 연습 훈련량은 확인된 연습 시간과 RPE를 반영한 흐름입니다."
        }
...
internal fun TrendMetricId.label(): String =
    when (this) {
        ...
        TrendMetricId.BADMINTON_PRACTICE_LOAD -> "배드민턴 연습 훈련량"
        ...
        TrendMetricId.BENCH_PRESS_E1RM -> "벤치프레스 posterior 중앙값"
        TrendMetricId.SQUAT_E1RM -> "스쿼트 posterior 중앙값"
        TrendMetricId.DEADLIFT_E1RM -> "데드리프트 posterior 중앙값"
        ...
        TrendMetricId.CORE_TOTAL_STIMULUS_WEEKLY -> "주간 코어 총 자극"
        TrendMetricId.CORE_DIRECT_STIMULUS_WEEKLY -> "주간 코어 직접 자극"
        TrendMetricId.CORE_INDIRECT_STIMULUS_WEEKLY -> "주간 코어 간접 자극"
