...
any calculator in this phase. Region and mode qualifiers use the closed
`PhysicalQualityRegion` and `PhysicalQualityMode` vocabularies. Atomic
qualifiers such as `POSTERIOR_CHAIN + HINGE` are preferred to compound free
text such as `HINGE_POSTERIOR_CHAIN` or `FRONTAL_SSC`; mechanical context stays
