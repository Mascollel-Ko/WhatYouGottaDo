...
| Implemented from app version | v0.4.2.8; relative presentation active from v0.4.2.12; EDU-2 dialog active from v0.4.2.13; diagnostics internal-only from v0.4.2.14 |
| Last audited commit | 9b9124d1d9cd5b6fcdd42a7578d2295f991bbe1b |
...
Connective-tissue summary, top-three joint, expanded child rows, contributor, educational dialog와 final provenance footer에 적용합니다. Diagnostics는 domain 내부 기록과 테스트에만 적용합니다.
...
UI는 전체 registry, calibration percentage나 diagnostics를 받지 않습니다. Domain aggregation이 만든 canonical status, EDU-2 educational info, contributor, recovery range와 explicit provenance만 presentation model로 받습니다.
...
- Diagnostics는 개발/모델 한계를 보조하는 내부 정보이며 사용자 화면에 표시하지 않습니다.
...
- Internal diagnostics excluded from UI state and rendering: `IMPLEMENTED / TESTED`
...
- `ConnectiveTissueAnalysisUiTest.kt`: exact text, diagnostics 비노출, footer 유지, exact three headings, parent/child access, large-font scrolling와 dark theme
...
- `2.1.1` (2026-07-18): 내부 diagnostics를 presentation model과 사용자 화면에서 제거하고 single provenance footer를 유지했습니다.
