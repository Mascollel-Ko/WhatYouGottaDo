...
절대 modeled exposure와 개인 history 내 상대 위치를 분리합니다. 충분한 기록 전에는 정밀한 개인 상태처럼 보이지 않게 calibration 상태를 표시합니다.
...
이 문서는 `CT-PERSONAL-CALIBRATION`가 소유한 현재 runtime 동작과 직접 연결된 source, tests, authority assets에 적용됩니다.
...
관찰 56일 미만은 `CALIBRATING`, history 없음은 `UNAVAILABLE`입니다. 개인 percentile은 90 초과 VERY_HIGH, 75 초과 HIGH, 50 초과 MODERATE, 그 이하는 LOW입니다.
...
개인 상태는 동일 load unit의 관찰 history에 대해서만 상대화합니다. 증상 override는 percentile을 바꾸지 않고 product-facing canonical state의 하한을 올립니다.
