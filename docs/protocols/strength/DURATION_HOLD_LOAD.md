...
plank와 side-plank 계열은 confirmed seconds와 RPE를 hold load로 변환합니다.
...
이 문서는 `STRENGTH-DURATION-HOLD`가 소유한 현재 runtime 동작과 직접 연결된 source, tests, authority assets에 적용됩니다.
...
- plank와 side-plank family만 명시 지원하며 정확한 최초 app version은 추가 감사가 필요합니다.
