...
OFI는 확인된 운동 기록에서 계산한 다섯 피로 축을 0~100 하나의 현재 상태로 요약합니다. 연결조직은 별도 회복 도메인이며 OFI 숫자 축에 포함되지 않습니다.
...
이 문서는 `OFI-CORE`가 소유한 현재 runtime 동작과 직접 연결된 source, tests, authority assets에 적용됩니다.
...
축별 상대 점수는 `raw / max(baseline, 1) * 50`을 0~100으로 제한합니다. OFI는 `round(0.45 * mean + 0.35 * max + 0.20 * highAxisPenalty)`이며 0~100으로 제한합니다. 80 이상 축 개수의 penalty는 0개 0, 1개 60, 2개 80, 3개 이상 100입니다.
...
최근 baseline이 작거나 없으면 분모를 최소 1로 두고 초기 profile seed와 관찰 기록을 현재 구현대로 혼합합니다.

## 11. 개인화 또는 보정

최근 14일과 그 이전 14일의 median baseline을 사용하며 초기 profile seed는 관찰 28일 동안 점차 빠집니다.
