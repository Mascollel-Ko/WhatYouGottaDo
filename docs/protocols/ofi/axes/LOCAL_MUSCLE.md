...
| Field | Value |
|---|---|
| Protocol ID | OFI-AXIS-LOCAL-MUSCLE |
...
`1.1.0`은 canonical OFI 축 집합에서 국소 근육 축의 경계를 재확인한 계약입니다. 과학적 완전성, 임상 타당성 또는 예측 정확도를 뜻하지 않습니다.

## 1. 일반 사용자용 요약

특정 근육과 부위에 집중된 set, reps, RPE 부담을 모델링합니다. 현재 primary detail은 기여 운동이 아니라 피로 근육 최대 3개입니다.
...
이 문서는 `OFI-AXIS-LOCAL-MUSCLE`가 소유한 현재 runtime 동작과 직접 연결된 source, tests, authority assets에 적용됩니다.
...
기여량은 `levelMultiplier * setModifier * repetitionModifier * rpeModifier * recordLoad * 50`입니다. 2세트 이하는 0.75, 6세트 이상은 1.20, 15회 이상은 1.10, RPE 9 이상은 1.20입니다.

## 8. 집계 방식

확인된 set/entry의 일별 raw contribution을 합친 뒤 decay를 적용하고 개인 baseline과 비교합니다. UI 축 이름은 `국소 근육`입니다.
...
- `1.1.0` (2026-07-18): canonical 다섯 축 복원 후 국소 근육 계산과 표시 경계를 재감사했습니다.
