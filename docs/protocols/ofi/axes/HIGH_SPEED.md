...
| Field | Value |
|---|---|
| Protocol ID | OFI-AXIS-HIGH-SPEED |
...
`1.1.0`은 legacy 관절·건·충격 축을 제거하고 실제 고속 workload 축을 구현한 계약입니다. 과학적 완전성, 임상 타당성 또는 예측 정확도를 뜻하지 않습니다.

## 1. 일반 사용자용 요약

`HIGH_SPEED`는 빠른 수축·이동·가속과 탄성 SSC 성격의 workload 부담을 모델링합니다. 연결조직 부하나 충격 조직 상태를 대신하지 않습니다.
...
이 문서는 `OFI-AXIS-HIGH-SPEED`가 소유한 현재 runtime 동작과 직접 연결된 source, tests, authority assets에 적용됩니다.
...
확인된 기록의 duration/RPE workload와 effective runtime metadata를 사용합니다. movement family/subtype, program slot, primary/secondary stress tags, sport context, badminton transfer type, skill target과 physical quality에서 `NEURAL_SPEED`, `SPRINT`, `MAX_VELOCITY`, `PLYOMETRIC`, `ELASTIC_SSC`, `ACCELERATION`, `FIRST_STEP`, `COURT_MOVEMENT`, `FOOTWORK`, `RUNNING`, `REACTIVE_AGILITY` 신호를 읽습니다.

## 7. 계산 또는 분류 계약

기여량은 `recordLoad * levelMultiplier * rpeModifier * velocityModifier * recoveryModifier * 50`입니다. high-speed 신호가 있으면 level을 HIGH 또는 조건부 VERY_HIGH로 올리고, sprint/max velocity 1.25, plyometric/elastic SSC/acceleration/first step 1.20, court movement/footwork/running 1.15, 일반 sport session 1.10의 velocity modifier를 적용합니다.

## 8. 집계 방식

확인된 set/entry의 일별 raw contribution을 합친 뒤 decay를 적용하고 개인 baseline과 비교합니다. UI 축 이름은 `고속`입니다.
...
고속 metadata 신호가 없으면 sport session은 MODERATE, 그 밖의 기록은 LOW fallback을 사용합니다. 확인되지 않은 세트는 계산에 포함하지 않습니다.
...
- `jointTendonImpactStressLevel`, tissue load unit, prior, `PersonalBaseline`, `w_perUnit`과 tissue recovery state는 이 축의 입력이 아닙니다.
...
- `1.1.0` (2026-07-18): 관절·건·충격 해석을 제거하고 독립적인 고속 workload 계산과 표시를 구현했습니다.
