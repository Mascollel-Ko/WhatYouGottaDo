...
## 2026-09-25 — B9.2 typed canonical failure-boundary closeout (protocol 3.44.2)

- Starting verified `main` was `23b3b6c78c26d4ea5732d39dece6ba0d4dfbc094`. Implementation commits were `64fd979b` (typed canonical failure model and producer adapters), `aee8ce90` (focused orchestration boundary coverage), `5d59b7dc` (internal seam/compiler correction), `ef7ba079` (Room exercise fixture seed), and `a76fb20e4662d72721590401e5822e2c48347e02` (suspend fixture correction). The final audited implementation/test SHA is `a76fb20e4662d72721590401e5822e2c48347e02`; protocol documentation is `3.44.2`.
- `StimulusCanonicalEvaluationFailure` carries an enumerated canonical reason and optional detail code. Known bounded producers now emit that typed failure for executable-demand absence, final canonical validation, and B6/regional/material authorization boundaries; the production adapter no longer parses `RuntimeException.message`, `substringBefore(':')`, or authorization string prefixes. Cancellation is rethrown first, unexpected programmer/data-system failures and CONTROL-generation failures propagate, and only the typed canonical boundary may return the already-valid original CONTROL object.
- `StimulusProductionFailureBoundaryTest` exercises injected typed expected failure, cancellation, unexpected `IllegalStateException`, unexpected `IllegalArgumentException`, and CONTROL failure. It verifies CONTROL identity, CONTROL routing and fallback reason, `1 / 0 / 0` pre-builder counts, monotonic completion only on expected fallback, no completion after cancellation, and persistence purity. Existing real authorized Strength routing, CONTROL_ONLY rollback, B8 authority flags, progress mapping, and one-CONTROL/one-EXPERIMENTAL/zero-third-build semantics remain unchanged.
- Local Windows pre-push compile and the required targeted test suite were attempted and stopped before task execution with `java.io.IOException: Unable to establish loopback connection`. Hosted implementation CI `36068580576` passed the complete workflow: 2,045 tests completed, 0 failed, 4 skipped; protocol validation reported 9 families and 36 protocols; Community/Cloud contracts, whitespace, debug APK assembly, signer verification and upload all passed. Artifact `WhatYouGottaDo-debug-apk` id `10837656442`, uploaded zip digest `sha256:d109f1083239c1990d44f030b836ca1e269b1536e54bfe7eadb679db053cd994`, downloaded APK SHA-256 `7627B9FB9B778E5368D44C27435084DD7BF1BA02A437DAF11D43AA5BC4FC6BB4`. The documentation closeout follows as a separate commit and CI run.

## 2026-09-25 — B9.1 production operational hardening (protocol 3.44.1)

- Starting verified `main` was `891d04148f4f6c87b4981ee381a54dfa4f78bb34`. Implementation/test commit `54cd41d3da19224f243fbc8e5af67db208ea604b` adds a production-only progress mapper, preserves the original CONTROL object on every B9 CONTROL route, and narrows experimental fallback to known canonical materialization failures. B8 authority semantics, the active `B8_STRENGTH_V1` scope, and the pure B9 router are unchanged.
- The progress mapper maps the whole operation into CONTROL `5–45`, EXPERIMENTAL `46–92`, selection/validation `93–98`, and completion `100`; it never resets or regresses the downstream reporter. Comparison-only prescription realization evidence remains on `StimulusSelectionProgramComparison`; CONTROL fallback therefore carries no B9 shadow plan into preview/save persistence. The build counter now derives `thirdBuilds` from recorded top-level program-builder invocations.
- The expected-failure boundary recognizes only bounded canonical materialization reasons (`B6_AUTHORIZATION_*`, `REGIONAL_AUTHORIZATION_*`, `MATERIAL_AUTHORIZATION_*`, `NO_EXECUTABLE_PLANNING_DEMAND`, and `FINAL_CANONICAL_VALIDATION`). Cancellation is rethrown before state/argument handling; unknown programmer/data-system failures and CONTROL failures propagate. Real integration coverage asserts CONTROL object identity and persisted decision JSON purity; the progress mapper test proves visible forward movement after CONTROL and completion-only `100`.
- Local pre-push compile and targeted tests were attempted and stopped before task execution with `java.io.IOException: Unable to establish loopback connection`. Hosted implementation CI `36024648464` passed the complete workflow: 2,040 tests completed, 0 failed, 4 skipped; protocol validation reported 9 families and 36 protocols; Community/Cloud contracts, whitespace, debug APK assembly, signer verification and upload all passed. Artifact `WhatYouGottaDo-debug-apk` id `10819153424`, uploaded zip digest `sha256:2c541fa9c3875416669c6d0034d9a7b8d3e8220d6e3d47e01bd41c83bc5beea2`, downloaded APK SHA-256 `F7730FF6A49D722A87F6EF5571A18BEE82DE6CD229FE07A0473ACF99432F4B52`. The documentation closeout follows as a separate commit.
- Final HEAD/docs CI `36026693674` on `eb811c382016883ac84ea79107b45143891e7174` also passed the complete workflow with the same 2,040/0/4 test result and 9-family/36-protocol validation. Its artifact id is `10820566477` with uploaded zip digest `sha256:9aed6800b50d7bd643925c9c3d6e422fd23a65b77fe91ed04ac162339aed39c2`.

## 2026-09-24 — B9.0 bounded Strength production routing activation (protocol 3.44.0)

- Starting verified `main` was `5b7b521bc5efe35d961f391cffdcce5716ba4a99`. Implementation commits were `59f4de8d` (distinct pure router and production orchestration), `fc86681f`/`679401ed` (router and real service/rollback coverage), and `ee21a91d` (B6 realization diagnostics retained while preserving the existing comparison contract). The final audited implementation/test SHA is `ee21a91d5de38dc2da4a8a122a30207612d88821`; the documentation closeout follows as a separate commit.
- B8 remains the authority oracle and was not changed into a router. B9 is a pure selection layer over the already-built `comparison.control` and `comparison.experimental`: the default internal policy is `B8_STRENGTH_V1_ACTIVE`, while `CONTROL_ONLY` is the explicit emergency rollback path. Only B8 `AUTHORIZED_FOR_BOUNDED_CUTOVER` in `STRENGTH_V1`, with non-empty authorized owners, B7 `ELIGIBLE_FOR_FUTURE_CUTOVER_REVIEW`, and both B8 contract flags false, returns the intact existing EXPERIMENTAL skeleton. All other B8 states, missing or malformed authority, wrong scope, and empty owners return the intact CONTROL skeleton with deterministic B9 reason codes.
- The real Room/service production entry point now runs one CONTROL build and one in-memory EXPERIMENTAL build, then performs zero builds for B9 selection. The authorized `barbell_back_squat` fixture proves routed fingerprint equals the existing EXPERIMENTAL fingerprint and differs from CONTROL; the removed-owner fixture returns CONTROL; the real authorized fixture under `CONTROL_ONLY` returns CONTROL while B8 remains authorized. `winner=null`, `routingActive=false`, and `productionMutationAuthority=false` remain unchanged. The selected skeleton continues through the existing preview/save/session-linking path; no saved programs are migrated or rewritten and no persistence, backup, Room schema, remote-config, user toggle or other-quality activation was added.
- Hosted implementation CI `36018646475` passed the complete workflow: 2,039 tests completed, 0 failed, 4 skipped; protocol validation reported 9 families and 36 protocols; Community/Cloud contracts, whitespace, debug APK assembly, signer verification and upload all passed. Artifact `WhatYouGottaDo-debug-apk` id `10815394323`, uploaded zip digest `sha256:ca12885c41fa0e9a8480e6d839f23eddeccd150342c420c253eca98666c9ac6a2`. Local Windows compile and required targeted tests were attempted before each push and remain blocked before task execution by `java.io.IOException: Unable to establish loopback connection`.

## 2026-09-24 — B8.1 pre-activation closeout (protocol 3.43.1)

- Starting verified `main` was `86d67c334d6092864bb4b795bd201aba0de57b86`. The B8.1 implementation commits are `7b22b220` (empty material-owner fail-closed and Strength numeric-authority guard), `492b865e` (boundary coverage and material-owner validation), and `368b2dd4` (Strength-only real service fixture); the final assertion cleanup commit is `556868a608b97169032fcee27e237953ca3bfa86`. The protocol audit records that final implementation/test SHA and bumps `PROGRAM-BUILDER-OVERVIEW` to `3.43.1`.
- The real Room/service entry point now proves the complete bounded Strength chain without manual B5/B6/B7 overwrites in the positive fixture: B4 Strength target authority → exact B5 `barbell_back_squat` owner → exact executable B6 `AUTHORIZED_SAFE_REPAIR` → every expected week `FULLY_MATERIALIZED` with zero shortfall/overrun and preserved/subset prescriptions → B7 `ELIGIBLE_FOR_FUTURE_CUTOVER_REVIEW` → B8 `AUTHORIZED_FOR_BOUNDED_CUTOVER`. The decision remains scoped to `STRENGTH_V1`, has the exact owner role `STYLE_HEAVY_LOWER_KNEE`, and retains `routingActive=false` and `productionMutationAuthority=false`; the blocked removed-owner service case remains `CONTROL_REQUIRED`.
- B8.1 fails closed when B7 is eligible but no material owner exists, emits `B8_CUTOVER_V1_EMPTY_MATERIAL_AUTHORITY` plus upstream inconsistency evidence, and never authorizes an empty owner list. Added and shared Strength material-owner paths reject B4 numeric authority `NONE` and `UNRESOLVED` with `B8_CUTOVER_V1_STRENGTH_TARGET_HAS_NO_NUMERIC_AUTHORITY`; valid direction-only authority remains allowed. No production routing, activation flag, persistence, schema, or third build was introduced.
- Hosted implementation CI `36005056750` passed the complete workflow on `368b2dd4`: 2,030 tests completed, 0 failed, 4 skipped; protocol validation reported 9 families and 36 protocols; Community/Cloud contracts, whitespace, debug APK assembly, signer verification and upload all passed. Artifact `WhatYouGottaDo-debug-apk` id `10809424260`, uploaded zip digest `sha256:1d87d2776e3eeac83810fb45b06dded1ccf697dddc26ebd2a6e175016d5d23fb`. Local Windows compile and targeted tests were attempted before the final push and remain blocked before task execution by `java.io.IOException: Unable to establish loopback connection`.

## 2026-09-24 — B8.0 bounded production cutover authority gate (protocol 3.43.0)

- Starting verified `main` was `44b5f659bd220654e6d0f63b25d8e6cb32c45169`. The B8 implementation commit is `0a2b33ca`; focused pure and service test commits are `a2de206b`, `543172c3`, `630897b2`, `994bdb4f`, and `4033d838`. The final audited implementation/test SHA is `4033d8381ef9cb16ba4e625123c9a9eb830ad34b`; canonical protocol documentation is `3.43.0`.
- B8 consumes the existing B7 audit and B6.2 comparison once and emits a typed `StimulusProductionCutoverAuthorityDecision` for `STRENGTH_V1`. `AUTHORIZED_FOR_BOUNDED_CUTOVER` requires B7 `ELIGIBLE`, exact `(stableKey, selectionRole)` B5 provenance, exact executable B6 Strength authorization, complete all-week materialization, preserved prescription fields, unchanged weekday schedule, and no collateral, unexplained, removed-owner, or inconclusive evidence. B7 `NOT_ELIGIBLE`, `INCONCLUSIVE`, and `NO_MATERIAL_CHANGE` map to `CONTROL_REQUIRED`, `INCONCLUSIVE`, and `NO_MATERIAL_CHANGE`; non-Strength material change remains outside the bounded scope. Owners and reason codes are deterministic and the compact JSON is diagnostic only.
- The service seam performs exactly one unchanged CONTROL and one in-memory EXPERIMENTAL build through the existing B6.2 path. Normal `generatePrepared` remains the CONTROL fingerprint; `routingActive=false`, `productionMutationAuthority=false`, `winner=null`, and no persistence, feature flag, user setting or production cutover were introduced. Pure coverage proves authorized Strength repair, additive Strength, removal blocking, partial/multi-week materialization, non-Strength scope boundaries, B7 mapping, B6 integrity, exact-role rejection and deterministic output. The real Room/service fixture proves the typed B8 decision and the blocked removed-owner fixture preserves `CONTROL_REQUIRED`.
- Hosted CI `35990604330` passed the complete debug workflow on `4033d838`: 2,026 tests completed, 0 failed, 4 skipped; protocol validation reported 9 families and 36 protocols; Community/Cloud contracts, whitespace, debug APK assembly, signer verification and upload all passed. Artifact `WhatYouGottaDo-debug-apk` id `10804068497`, uploaded zip digest `sha256:89e3711f3b756c183e3fc4308b244cd4e275174b537c13e090151973f1955332`. Local Windows Gradle compile and targeted tests were attempted before the push and remain blocked before task execution by `java.io.IOException: Unable to establish loopback connection`.

## 2026-09-24 — B7 owner-local removed-identity provenance closeout (protocol 3.42.2)

- Starting verified `main` was `c7808015f9f972b833a60c99f960b57fd4b663dc`. Implementation commit: `41d50990` (`StimulusCandidateSelectionTrace`/materialization role identity and exact comparison owner sets plus owner-local B7 causal evidence); test commits: `a902c1c3`, `71d4ea83`, and `8b657be9`. The final audited implementation/test SHA is `8b657be90f89b370df70d04502f6a45057a1aa8b`.
- B7 now evaluates each removed CONTROL `(stableKey, selectionRole)` owner independently. Global B5/B6 material change is allowed only as existence evidence; capacity, placement, reflow and disappearance must come from the removed owner's selected/reused/rejected/materialization trace. Unrelated target trace codes cannot prove displacement. Owner-local proof yields `DOWNSTREAM_CONSTRAINT_DISPLACEMENT`; governed participation without local disappearance yields `INCONCLUSIVE_DISPLACEMENT` with `REMOVAL_CAUSALITY_UNPROVEN`; no governed causal path yields `UNEXPLAINED` with `UNEXPLAINED_REMOVED_IDENTITY` and `NOT_ELIGIBLE`. Added identities also require exact role-matched B5 authority.
- Hosted CI `35982579364` was the final successful run for this release: `2,018 tests completed, 0 failed, 4 skipped`; protocol validation reported `9 families, 36 protocols`; Community/Cloud contracts, whitespace, debug APK assembly, signer verification and upload all passed. Artifact `WhatYouGottaDo-debug-apk` id `10801415571`; uploaded zip digest `sha256:b2a899fc3a36c4ab99b54bc22c9a16e47fc508988496a7f5d450040b32b43b10`; downloaded APK SHA-256 `15FFFA14D5619407C464F135EC4CB023F6B909B0632424988EE32E7F47BEE029`.
- Local Windows Gradle was attempted before each push and remained blocked before task execution by `Unable to establish loopback connection`. CONTROL fingerprint parity remains exact; build counts are one CONTROL and one in-memory EXPERIMENTAL, `winner=null`, `productionAuthority=false`, normal `generatePrepared` is unchanged, B6 exact-set and numeric semantics remain unchanged, and production cutover remains inactive.
...
- B1/B2 now share the typed `QualityObservationClassification` helper. Strength/Hypertrophy require reviewed source plus `REALIZED` or `REVIEWED_NON_REALIZATION`; realization `UNCLASSIFIED` is partial relevant evidence. POWER/RFD/SSC/endurance/cardio/mobility remain source-and-relation capability proxies. A source without a physical-quality relation is irrelevant to quality counters while independent task evidence remains intact.
- B6.1 keeps exact `(stableKey, selectionRole)` ownership, fails closed for ambiguous B5 identities, and covers the CONTROL matrix (one compatible owner, multiple compatible owners, and no compatible owner). The real service comparison now seeds a reviewed effort-backed Strength history and exercise-local posterior for one canonical squat owner, producing a safe shadow proposal with preserved set count and no load increase. The 69.9/70.0/80.0% planned Strength boundaries are explicit.
- Normal `generatePrepared` output and CONTROL fingerprint assertions remain unchanged; B6.1 is shadow-only with `mutationAuthority=false`, and B6.2 remains inactive. Local Gradle remains blocked before task execution by the Windows Java loopback error; hosted CI is authoritative. Normal hosted CI `35839307267` passed `testDebugUnitTest` (1,996 tests, 0 failures, 4 skipped), assembled and signer-verified the debug APK, and uploaded artifact `WhatYouGottaDo-debug-apk` (artifact id `10740752731`, digest `sha256:ab957912ae25509f6cd67c93e99aa629945f31c9ae7f3032fb059d9e1e2070a0`). APK SHA-256: `40a5fd126f2b84b95fd153fe53298da41c668af30c6f696de14b3efee230126d`.

## 2026-09-23 — Phase B6.0 reviewed realization and B6.1 shadow prescription resolution

- Baseline verified at `754b40d858c33aab60e7b798764ad7266051e36e`, with the A–B5 closeout clean and `origin/main` aligned before implementation. Implementation commits are `61ada6dab68c6b129afc858dd9c0c8b8938776cd`, `eb5831c8ccf8337a3937709dbf8d21f0a3f3f938`, `a366637ffc95a97ea11fa831c1b342d135b504c2`, `a9bfa4be5b40ab157595aadfc8cf2b1d1bbdd21e`, `be822c1fa5f7d2ca98e7f00c4c62bca1ff05917`, and `c385c1c2df81948bd04555d1f83fce8a51e0e76f`; the documentation registry is version `3.40.0`.
- B6.0 adds one typed `RealizedStimulusClassifier` for reviewed resistance identity, direct canonical Strength/Hypertrophy relation, rep band, independent reference 1RM, canonical load resolution and RPE/RIR effort gates. Same-session `priorLogMean` is used only with `baselineEstablishedBefore`; otherwise the reference index is strictly historical, with no future or same-set posterior leakage. Reviewed non-realization and realization `UNCLASSIFIED` remain distinct. POWER/RFD/SSC stay capability proxies.
- B1/B2 and historical regional shadow consumers now consume typed classifications. B6.1 adds `StimulusPrescriptionRealizationPlanEngine` with exact B5 owner `(stableKey, selectionRole)`, current/probe/proposed typed prescriptions, effort targets, separate shadow authority and explicit fail-closed statuses. It is attached only to the explicit B5 comparison shadow result; normal generation, program mutation, persistence, fingerprints, set counts, scheduling, progression and winner selection remain unchanged. The Hypertrophy `8 reps / 0 kg` path remains a typed proposal only.
- Added focused classifier coverage for six Strength cases, five Hypertrophy cases, temporal authority, non-realization/unclassified observability and proxy boundaries, plus B6.1 compatible, incompatible, unavailable-load, provisional, proxy and ambiguous-owner cases. Local Gradle remains blocked before task execution by the Windows Java loopback error; hosted CI is the authoritative build/test path. Hosted code run `35807081033` passed on `c385c1c2df81948bd04555d1f83fce8a51e0e76f`: 1,986 tests completed, 0 failed, 4 skipped; debug APK artifact digest `sha256:cad24cc5235f64be7f3ca5670ad827b979ba9f53b16379415da73961a0a944ea`, APK SHA-256 `B0FF4256CDFC90DB28B2CA79A9EF018B69D9D5AB47AD18335A6252B4211F8E35`.
...
- Pre-edit path inspection: MaterialDemandResolver uses existing selector, equipment/exclusion/tissue/time gates and one legitimate candidate per owned gap (foundational gaps may select several). It creates candidate targetSets; typed merge separates exact regional owners while ordinary merge preserves existing max-of-request semantics. Material candidates sort resistance-first, descending priority, stableKey; at most one existing nonmaterial optional candidate follows funded continuity/material work.
- Frozen FiniteExecutionAllocator atomically admits full minimums inside capacity minus core reserve, then raises ONLY already-selected flexible candidates round-robin until max(spent, round(capacity * structure share)). Deferred indexes are never reconsidered in that loop. Regional indexes are excluded from flexibility; ordinary resistance indexes remain flexible. gapItems copy those enlarged integers into targetSets; capacityCandidateTrace then re-prescribes funded quantities. This is the exact candidate inflation path.
- No independent per-candidate expansion maximum was found: ResistanceVolumePlanner's normal-week baseline, day release, time/useful-demand ceilings are aggregate volume bounds, not authority to enlarge a particular material candidate. Canonical prescriptions normally use targetSets directly. Novel resistance prescriptions require at least two sets; reviewed performance prescriptions can be indivisible. Those are shape/atomicity constraints, not spare-capacity expansion authority.
- FrequencyDemandProvenance carries original prescriptions and capacity-rejected residuals. FrequencyExpansionPlanner considers these before independently authorized retained incumbents and uses subsets/compatible re-prescriptions; extra days bound executable release, not original demand. ResidualCompletion restores exact parents first, but its later semantic substitute loop has same-key rather than per-owner ceilings and may create alternatives. It needs a bounded experimental owner boundary. Placement/rebalance/reflow can move/drop/restore authorized shapes; they cannot authorize additional demand.
- Test-first fixtures call actual PersonalizedProgramBuilder.buildCore through its existing capacityOverride seam, exercising real prescription, placement, residual completion and balancing. The public real-builder regional hard-cap regression remains unchanged. Cases 19/(5,6,8) and 19/(5,4,4) without continuity already have correct numerical totals in the frozen kernel because its share ceiling is <=62%; they must not be falsely claimed as numerical inflation reproductions. New audit-contract assertions and separate selected-ordinary inflation/deferred fixtures distinguish those findings.

- Pre-production RED: five actual-builder tests, four failures and CONTROL pass. Deferred capacity19: regional A5, ordinary B5 (max4), C0 (max10), total13. Exhausted capacity40: A5/B9/C8, total25 (ordinary maxima4/4). Cases A/B already numerically19/13, but failed the missing maximum/unused-capacity audit contract. CONTROL fingerprint captured before editing production: `dee9d47df5990278160a076c6627c1cb16f7ffd7d2778505e0c15a7ab9d2fb7c`. Private red XML/log retained outside Git.

- Implementation: experimental `BoundedMaterialDemandAllocation` resolves exact regional max or original ordinary targetSets max, rejects canonical shapes above that max, invokes the unchanged atomic finite pass with no flexible expansion, and reconsiders deferred owners via existing canonical `frequencyPortion` rules. Equipment/exclusion/metadata/tissue gates and downstream feasibility remain active. Original bounded prescriptions travel through frequency provenance; experimental residual completion permits exact parent restoration only. Per-owner funded/materialized checks run after completion and final frequency/reflow. CONTROL returns through the original path without the bounded audit; frozen files/hashes are unchanged.
- Initial GREEN: all 12 new regressions plus all 12 existing RegionalAuthorityBoundsTest tests passed. Case A capacity19 -> A5/B6/C8, unused0; B -> A5/B4/C4, unused6; capacity10 -> A5/B4/C1; deferred -> A5/B4/C9 plus continuity1; exhausted capacity40 -> A5/B4/C4 plus continuity3, unused24. CONTROL's captured synthetic fingerprint remains exact. Supplemental coverage verifies public-builder explicit frequency, original ordinary subset release, exclusion, canonical minimum rejection, indivisible variant deferral and exact ordinary restoration without semantic filler.
- First required focused run: 112 tests / 13 classes, zero failures/errors/skips, including the actual private-backup comparison, exact restoration, CONTROL parity, isolation and freeze checks. The final observation-only refinement distinguishes total unfunded legitimate demand from the bounded-material remainder; the full run was deliberately interrupted to rerun final-source focused/real and full verification, not recorded as a successful run.
- Real backup: CONTROL exact rows/request/fingerprint match 62078a92 (`805c90244c1bac27218aad2f7003566c80b790d2b21417716b973bfdc2620c8f`). Experimental material bench owner now requests/maxes/funds/materializes2 instead of13, with identical canonical reps/load/rest/source. Capacity35, legitimate demand40 (= material2 + original continuity35 + optional3), funded/materialized35 (= material2 + continuity33), unused0, total unfunded5, bounded-material deferred0, capacity overrun0. Experimental fingerprint `8dbe76bb191eeb55b245f21d89321f3a562c2abe8ab69e530965bbae9124318f`; A has31 resistance +2 badminton +2 athletic; B has30 +2 +3. Differences follow bounded material demand and restored original continuity, not forced regional changes. All seven regional diagnoses retain recovery HOLD, authorization/materialization/overrun0.
- Separate diagnostics: strict final OFI days A=78/93/78/95/77 (five infeasible), B=84/96/95/94/76 (four infeasible, Sunday feasible). Numerical loads/cautions change with legitimate redistribution; this is not a claim of strict OFI feasibility. Tissue has no blocked units and the same unresolved dead-bug key `ex_d5bdffe1` (moves from Tuesday to Thursday); weekly tissue remains false. These pre-existing consistency issues are not repaired or hidden. Optional real-backup E2E rerun still fails the unchanged `Explicit anti-rotation assistance must actually execute` assertion; the test and authority are not weakened. Required focused verification is green; the optional diagnostic remains separately reported.


- FINAL verification on the final source: focused 112 tests / 13 classes, 112 passed, 0 failures/errors/skips, including the real private-backup A/B and exact prior CONTROL rows/request/fingerprint comparison. Full `testDebugUnitTest`: 1,899 tests / 307 classes, 1,895 passed, 0 failures/errors, 4 unchanged optional-private-artifact skips. `assembleDebug` PASS (combined full-test/build run successful). Protocol validator PASS (9 families / 36 protocols); Community/Cloud Node contract tests 9/9 PASS; `git diff --check` PASS. External Windows test JVM isolation (Temurin17, forkEvery1, two forks) is retained outside Git; no repository runtime or freeze manifest changes. All 12 new tests and all 12 regional-bound tests pass. Test results, per-owner synthetic/real audit and known optional anti-rotation failure are included in the delivery artifacts; private backup/raw logs remain outside the repository.


## 2026-09-20 — Regional hard maximum, owner attribution and recovery intervention gate

- Start SHA: `4dab3209d5bfec859ef1c2a8d6598426880c4719`, fetched/reset main matched origin/main with a clean tree before edits. Same private backup used for the preceding audit is retained outside Git; no backup, roundtrip, private JSON or TXT is part of this change.
- TEST FIRST: before any production edit, `RegionalAuthorityBoundsTest` ran 10 tests: 7 intentional assertion failures, 3 passes. Actual red XML/log retained privately. Real builder spare-capacity regression requested/authorized 5 but funded 19 (expected <=5, exactly 5 when feasible); merge ordinary 9 + regional 5 produced regional 9 (expected separate 9/5); prescription request 9 manufactured 9 from authorization 5 (expected rejection/subset); projector ordinary 2 + regional 5 reported 7 (expected 5). Stable, negative and insufficient response plus current recovery each returned RESTORE_STRENGTH_EXPOSURE (expected HOLD_FOR_RECOVERY). Positive response preserved, and non-recovery exposure/morphology decision controls passed before the fix.
- Prior exact-backup evidence identifies the actual 5 -> 9 -> 11 path: candidate trace still requested 5 and carried a 5-set regional prescription, but finite material funding was 9; `gapItems` adopted 9 and `authorizedPrescriptionFor` cycle-copied the 5-set source to 9. The final stableKey-wide projector then added 2 ordinary sets and reported 11. AUTO frequency was unchanged, so expansion did not cause that overrun. StableKey merge was a separate reproduced latent route to relabel ordinary quantity as regional.
- Fixed orchestration only: exact regional owners remain separate during merge and are excluded from the frozen allocator's flexible expansion index set. Authorized prescription lookup rejects requests above the original size; subset contents/rest/source remain exact. Capacity traces and frequency expansion retain the earlier typed, subset-only provenance fix. Exact residual restoration uses authorized parents; semantic completion cannot re-prescribe bounded regional owners. Post-split/reflow preserves parent identity and a per-week assertion checks the final untrimmed total against the original maximum. `ExecutionAllocationPlanner.kt` and finite allocator semantics are unchanged.
- Final projection follows canonical `authorizedDemandId` where available, otherwise exact `RegionalSelectionIdentity`, never stableKey alone or role substrings. It reports actual compatible regional units, ordinary same-key units, shortfall and overrun; invalid 6/5 audit reports 6 and REGIONAL_AUTHORIZATION_OVERRUN with overrun 1, without hiding the excess.
- Decision order preserves positive response first; otherwise current recovery constraint gates HOLD before sport/specificity/exposure/morphology. Causal diagnosis is unchanged. HOLD has NONE numeric authority and zero new residual/authorization/materialization. Added nonvacuous multi-region ADD/RESTORE invariant coverage, all-real-traces assertions, exact CONTROL baseline comparison, and 5-authorized/3-funded frequency subset regression.
- Focused final verification PASS: 101 tests across all 13 required/new classes, 100 passed, zero failures/errors, one existing optional private-Python parity skip. New bounds suite: 12/12 passed, including each original red assertion and the added global multi-region/subset checks. Initial added multi-region fixture produced NO_EXECUTABLE_PLANNING_DEMAND because its sparse history provided no scheduling capacity; supplied deterministic sustained history, leaving all assertions and production code unchanged, then reran all 13 classes successfully.
- Same-backup final audit PASS, including save/restore annotations and full CONTROL week-one row/request/fingerprint equality with the previous private audit. CONTROL fingerprint remains `805c90244c1bac27218aad2f7003566c80b790d2b21417716b973bfdc2620c8f`. All seven regional traces now HOLD under current recovery plus insufficient response, with residual/authorized/materialized/overrun all zero. Ordinary compatible strength work remains 2 units in each of LOWER_KNEE and POSTERIOR_CHAIN. Both A/B have 31 resistance sets, zero regional additions, 2 badminton and 2 athletic bouts; A/B final rows are identical. Quality capability views remain overlapping, not additive volume.
- Strict final feasibility remains a separate pre-existing follow-up: both A/B fail day OFI checks and tissue projection (unresolved exercise metadata); this task does not alter those authorities or claim production readiness for B. Detailed private A/B program, target/owner and feasibility reports are outside Git.
- Opt-in real-backup E2E rerun: FAIL at the unchanged `Explicit anti-rotation assistance must actually execute` assertion, matching the previously documented baseline. The 3-day program contains `cable_pallof_press`, selection role PERFORMANCE_CONTINUITY, candidate audit MATERIALIZED_CONTINUITY; band Pallof/landmine anti-rotation are FEASIBLE_ALTERNATIVE. `supportiveGapCodesByStableKey` is empty. Current gap is BADMINTON_DEVELOP_ANTI_ROTATION (LOW/nonmaterial), while the assertion requires BADMINTON_UNDERREPRESENTED_ANTI_ROTATION in executed support provenance. This is an existing gap/provenance expectation mismatch requiring separate follow-up, not absence of the exercise and not a reason to weaken the assertion. The opt-in diagnostic failure is distinct from the standard suite and required focused run.
- Final standard `testDebugUnitTest assembleDebug` PASS on the final production source: 1887 tests across 306 classes, 1883 passed, zero failures/errors, four unchanged opt-in/private-input skips; APK assembly succeeded. This includes historical CONTROL/Legacy parity, authority freeze, isolation and all 12 new bounds regressions. Local Windows execution uses an external JDK 17 per-class test-worker init script; repository build/test configuration is unchanged. The earlier incomplete full run was superseded after adding the new identity/overrun fields to audit JSON serialization.
- Final-source exact-backup audit rerun PASS (1 test, no skips), including unchanged CONTROL full fingerprint/representative rows, every regional trace invariant and recovery HOLD assertion. Protocol validator (9 families/36 protocols) and git diff --check passed. Local result logs/XML and private detailed reports remain outside Git; push-triggered final-SHA CI is verified separately.
- First post-edit compilation found a missing ProgramSkeletonItem import in the new owner helper; corrected before rerunning. This was a compile failure, not a regression green result.
- Protocol PROGRAM-BUILDER-OVERVIEW is 3.27.0; registry and index agree. Protocol validator passed (9 families, 36 protocols). Final test and real-audit outcomes are recorded above. Experimental B is not promoted; OFI/tissue and the pre-existing anti-rotation assertion remain separate diagnostics.


## 2026-09-14 — Record single-confirmation persistence

- Baseline: clean main == fetched origin/main at `5e578b019dcc6a7ca72fb6256d7044d7fd1b7df5`.
- Root cause: Record row input/focus-loss callbacks sent entire captured WorkoutSet copies. A late numeric edit could restore an old confirmed=false after the confirmation transaction had already persisted firstConfirmedAt and incremental displayOrder. Ordering did not lose its write; confirmation was overwritten independently.
- Record UI now sends explicit RecordSetEdit field intent, including bulk edits and suggested weight. RecordMutationService merges only those fields into the latest stored set inside the existing confirmation/order transaction. Confirmation changes only for explicit CONFIRMATION intent; bulk copy-values does not include it, copy-values-and-status does. No ID or source/binding field is copied from a UI snapshot.
- Current displayOrder remains presentation authority. Only the newly performed entry is inserted after the last other started entry (or first); manual drag and relative ordering remain unchanged. Explicit unconfirmation remains supported; timestamps do not become an ordering authority.
- Existing numeric normalization and focus-transition suppression remain intact; a fresh-storage no-op returns without DAO writes or derived refresh. No debounce, delayed reorder, optimistic confirmation, schema, backup, timer, planner, numerical analysis or progression-policy change.
- Reproduction: the lower-C regression using the former whole-set call passed its ordering assertion but failed confirmed=true (actual false); 9 tests / 1 intentional red failure. Switching that callback to REPS intent fixes the same sequence. Added all-field stale-callback, explicit-unconfirm, multiple-set identity, unchanged-order, Compose callback-intent and file-backed DB-close/reopen coverage.
- `./gradlew testDebugUnitTest --tests '*Record*' --no-daemon`: 117 passed, zero failures/errors/skips. Protocol validation: 8 families / 34 protocols passed. Localization authority passed; localization audit initially detected stale source-line references, then official `localization_audit.py --write` regenerated only the UI inventory and revalidation passed. No translated text changed.
- `./gradlew testDebugUnitTest assembleDebug --no-daemon --continue`: SUCCESS, 1,706 total / 1,702 passed / 0 failed / 0 errors / 4 optional-input skips (FrequencyExpansionRealBackupTest, TrainingStateParityTest Python-fixture case, RealBackupPersonalizedPlannerE2eTest, TrainingStateRealBackupComparisonTest). No optional inputs were enabled for this unrelated Record fix. Legacy frozen parity, planner isolation and existing personalized parity passed. Local Windows JBR test execution used JAVA_TOOL_OPTIONS=-XX:TieredStopAtLevel=1; no runtime/build policy changed in the repository.
- `git diff --check` passed. Excluded production authorities remain unchanged. The exact lower-entry / one confirmation / move / still confirmed / database reopen scenario passes in persistence tests; the actual row callbacks are covered by Robolectric Compose, not a physical-device claim. No standalone Record mutation/order canonical protocol exists; the existing ordering worklog is updated rather than creating a new protocol. No remote push requested or performed.

## 2026-09-13 — real progress through final reflow

- Fetched and fast-forward checked clean main == origin/main at e14fa47968f13d956eb33bbdee7ae3f34b9d26b2. Scope is generation progress only; previous summary/editor UI and planner numerical/search authorities remain unchanged.
- Compressed pre-reflow milestones below 65 (5/10/15/22/30/38/44/49/55/60/62/63/64), reserved 65–96 for applicable reflow, retained FINAL=98/COMPLETE=100. Small transient update overload preserves existing stage reporter callers and NONE. Runner applies the existing monotonic guard to both overloads; no persistent field or database change.
- Real events: applicable entry 65; representative ready 68; reference OFI/tissue validation 70; first round starts 72, completed row comparisons through 84; existing lazy tissue validation 84–89; accepted move 90; later rounds 90–92 and validations 93–94; mirroring/final checks 95; successfully returned result 96. Later unknown rounds saturate without going backward and may update phase messages. No fake timer, extra candidate/gate evaluation, changed ordering, altered cache, new search limit or simulated fast-path work. Optional candidate x/y display omitted.
- Added active/NONE full reflow result and operation-count equality test, real validation event and second-round assertions, monotonic update tests including virtual-time inactivity, stage-range/no-timer guard, and large-font progress rendering through 98/100. Existing exhaustive-reference, no-split and 29-persona tests remain unchanged except replacing the obsolete single-96-milestone expectation.
- Added eight Korean/English phase messages through the canonical localization exporter; summary hierarchy, Details behavior, editing scope and exercise rows are untouched. Protocol patch 3.16.2 documents transient execution milestones only.
- Focused run passed 40 tests: reflow 17, runner 6, progress UI 7, historical planner parity 10 (29 personas), zero failures/errors/skips. Active/NONE complete result and operation counts match; actual tissue callbacks occur below 96, a second round is observed after a move, and progress is monotonic through 98/100. Added a separate 320dp/fontScale 1.3 validation-message overflow test to the full run; 411dp/fontScale 1.3 milestones already passed. No candidate x/y UI was added.
- Localization regeneration shifts many generated-catalogue inventory line references and chunk boundaries. CSV comparison excluding sourceLineOrSymbol confirms zero old semantic rows removed/changed and only 26 added inventory rows; no unrelated translations were rewritten. Deterministic authority/audit and protocol validation passed.
- Full `testDebugUnitTest assembleDebug --no-daemon --continue` passed: 1701 total, 1697 passed, zero failures/errors, 4 unchanged optional private/Python input skips. Focused classes in the full run: reflow 17, runner 6, progress UI 8, historical parity 10; all passed, including new 320dp large-font message overflow test. Legacy 360 matrix, historical 29-persona fingerprints, source freeze/isolation and existing progression/Q/C/R tests passed unchanged. APK assembly, protocol validation (8 families/34 protocols), localization authority/audit and git diff --check passed. Local C1 JVM option retained to avoid the previously diagnosed Windows JBR C2 crash; no repository runtime workaround. No device timing/thermal claim, no golden updates, no release keys or tags.

## 2026-09-13 — Stage 3: structured and collapsible planning summary

- Started from clean main == origin/main 3f4ba4eab0e4443e2f4ff2659ebe140a72762dbb only after the Stage 2 push/fetch verification. Stage 2 hosted run: 34700986649.
- Replaced raw decision rendering with PlanningSummaryPresenter -> five-section PlanningSummaryUiModel -> PlanningSummaryCard. Observed classifications/confidence, identified needs, actual transition actions, numerical allocation and meaningful limitations have separate typed owners. The card defaults collapsed, supports Details/Collapse, and saves expansion state.
- Movement needs merge exact MovementCoverage keys across gap, representation and secondary sources; badminton needs merge exact canonical objective keys including drop/development/underrepresentation protocol tokens. Exercise transitions merge exact stableKey, retaining distinct structure/dose actions. Evidence-source sets are retained for tests. No fuzzy/name parsing or changes to the domain decision.
- Known GAP fallback tokens only fill missing needs; exact reviewed legacy constraints map to localized limitation categories. Unknown raw reasons/debug strings remain in audit data rather than leak into UI. Generic positive observations are excluded from limitations. Typed trace still exposes high-OFI split cautions, exact shortfall/rollback and session limits; minimal expansion does not invent an unavailable numerical amount.
- Allocation preserves existing numeric budget fields, including fractional baseline sets, structured badminton and athletic-performance bouts. No mutation of generated rows, prescriptions, Q/C/R, original fingerprint or planner math. Korean and English presentation records generated through the official authority; no hand-edited generated English resource.
- Initial focused build caught an invalid test import of the member getOrElseNullable, not a production compilation error; removed the import and reran. New tests cover semantic deduplication/section ownership, neutral unknowns, input immutability, saveable collapsed/expanded state and Korean/English 320/411dp at fontScale 1.0/1.3 with text-layout overflow checks.
- First executable focused run: presenter 7 passed, Compose 1 passed/1 failed. Diagnostic rerun measured title size 103x24 versus paragraph width 288 (width overflow, no height overflow). Summary text rows now explicitly fill the available width rather than use tight intrinsic width; no truncation, relaxed overflow assertion or narrower font workaround. The same layout test is rerun after this production presentation fix.
- Focused rerun passed all 9 tests (7 presenter, 2 Compose), zero failures/errors/skips. Layout test covers all 8 Korean/English x 320/411dp x 1.0/1.3 combinations with unchanged overflow and line-count assertions. Details state survives saved-instance restoration and exposes exactly five section tags before collapsing. Stage 2 hosted run 34700986649 completed successfully.
- Full `testDebugUnitTest assembleDebug --no-daemon --continue` passed: 1698 total, 1694 passed, zero failures/errors, the same 4 optional-input skips (FrequencyExpansionRealBackupTest, TrainingStateParityTest optional Python fixture, RealBackupPersonalizedPlannerE2eTest, TrainingStateRealBackupComparisonTest). Historical 29-persona and Legacy 360 parity, source freeze/isolation and progression persistence passed. Protocol validation (8 families/34 protocols), deterministic localization authority/audit and git diff --check passed. Stage 3 data/analysis production directories are byte-unchanged from its baseline; no goldens or source-freeze hashes were updated. UI evidence is Compose/Robolectric, not an actual-device screenshot or thermal benchmark.

## 2026-09-12 — Stage 2: generated Record-Based structural editing scope

- Started only after Stage 1 ca3b76983396348be8bf9c668c90b492f3752712 was pushed, fetched and matched clean main. Stage 1 hosted run 34700219350 subsequently completed successfully.
- Generated Record-Based preview defaults to Apply to all weeks and hides the week selector; individual-week mode restores it. Scope is saveable UI state. Non-isomorphic or ambiguous drafts force individual-week mode with a localized explanation, without overwriting data. Legacy and the existing generic saved-program editor retain their previous routing.
- Extended RepresentativeWeek with editor-only structural correspondence. Exact authorized parent/chunk plus canonical stableKey/variant/template slot own correspondence; unowned duplicate keys are rejected as ambiguous, not matched by name/order. Existing planner derive/mirror is unchanged. Structural edits map to each original row, never clone week-one prescriptions or bindings. New manual rows get distinct local IDs and use existing progression reconciliation.
- Apply-all add/delete/day move/order/day selection operates on relative weekdays. Set/rep/load edits remain current-week-only and are labeled as such. Moves/order retain each week's prescriptions, local IDs, bindings, session keys, split origin and original scheduling audit; explicit user deletion removes the requested rows without rewriting original authorization. Actual edited coverage is not falsely claimed unchanged after deletion.
- Focused tests: 20 passed, zero failures/errors/skips (scope model 5, narrow Compose/saveable scope 2, editor model 3, progression persistence 9, split-parent progression 1). Four/five-week fixtures cover structural actions and varying prescriptions; Room save/reload verifies moved rows retain week-local doses and progression session/logical IDs. Initial test compilation caught unsupported DpRect.width usage; corrected the test to right-minus-left and reran successfully.
- Korean/English scope, divergence and current-week prescription copy added through the localization authority; regenerated assets and localization audit passed. Protocol 3.16.0 documents the new editor semantics; registry audit SHA is the input Stage 1 commit. No schema, physiological formula, generated planner decision or Legacy source change.
- Full `testDebugUnitTest assembleDebug --no-daemon --continue` passed: 1689 tests, 1685 passed, zero failures/errors, the same 4 optional private-input skips; APK assembly passed. Historical 29-persona and Legacy 360 parity, source freeze, isolation, split/Q/C/R and progression suites passed unchanged. Protocol validation (8 families/34 protocols), localization authority/audit and git diff --check passed. No actual-device visual run claimed; narrow 288dp content / 320dp configuration at fontScale 1.3 was verified with Compose/Robolectric.
...
- Eligible canonical resistance items expose the compact progression-session row at 0kg. Activity/progress behavior/training-role authority replaces positive-load UI gating. Existing-session choices show prescription/role and deduplicated localized weekdays; role/mode/rule fields load the selected session's actual settings. New-session selection creates a new UUID. Disconnecting one member does not turn off other members' shared suggestion mode.
- Reused Room 31 and the existing six execution tables, backup format 13 / restore schema 12 / program schema 2; no schema, migration or backup-wire change. App `0.5.1.4` / code `501004` and release tags unchanged. Runtime `RECORD_BASED_PLANNER_0.14.1_KOTLIN_1`, PROGRAM-BUILDER-OVERVIEW `3.4.1`. Updated the existing spec §26, overview, registry/index and release note, without a duplicate canonical document.
- First network-enabled Gradle command was `--version` with Android Studio JBR and local `.gradle-user-home`. Focused progression/layout testing passed 57 tests before final regression additions. First full run passed 1,431 tests / 249 suites with 0 failures/errors and 3 conditional private-input skips. Final-source rerun results are recorded below.
- Initial test corrections were explicit: an old test constructed a lossy editor and relied on the removed hidden DB recovery; it now uses actual hydration. Named legacy SQLite hit a Windows WAL/path issue; reused the repository's native SQLite/short temporary context test procedure. A copy assertion used a date containing another legitimate program occurrence; its destination now has no pre-existing entry. No production move/copy policy was changed to satisfy the test.
- Android 17 emulator component instrumentation passed. Inspected all six actual manual-session sheet captures at 320/360/411dp × fontScale 1.0/1.3, including both prescription labels, weekdays, Main/App fields, short connection controls and save. Full-width vertical content has no one-character-per-line labels or clipped short controls. Robolectric tests additionally inspect actual line/glyph extents, role/mode popup choices and opening/saving a new session at 0kg. Captures remain ignored under `build/progression-v0141-verification`.
- Localization source/export determinism and audit passed; 10 Python localization tests passed; no unexplained English-mode Korean leaks or placeholder errors. Protocol validation passed for 8 families / 34 protocols. Generated localization inventories were refreshed with the official tooling. No compatibility inventory expansion was needed: the new code does not add a legacy `progressMetricType` consumer.
- Scope verification: `ProgramProgressionEngine`, all physiological `analysis/` sources, legacy `ProgramAutoBuilder`, `CalendarRecordService`, `RecordMutationService`, canonical metadata assets, Room schemas, backup implementation/wire and app version files are unchanged from baseline. No name/stableKey-fragment classifier was added. New AUTO draft comparisons are finalized with the existing canonical e1RM snapshot comparator at first save; explicit or already-persisted bindings remain authoritative. No threshold, generated-style drift, predecessor, RPE default or load-step redesign is included.
- The editor obtains eligibility and existing canonical e1RM snapshots together through `ProgressionDraftContext`; UI and save therefore use the same comparator inputs, including for AUTO with custom rules. The existing projection is shared, not recalculated by a new formula. Previously persisted null snapshots remain null. Headless/generated drafts without this context retain the existing first-save snapshot fallback.
- Final-source `testDebugUnitTest`, `assembleDebug` and `assembleDebugAndroidTest` PASS: **1,432 tests / 249 suites, 0 failures/errors, 3 conditional private-input skips**. The final suite includes **60 progression cases**: draft 9, engine 23, existing persistence 15, session persistence 6 and layout 7. Named DB restart, unchanged graph save, full custom rule, four modes, explicit same-key links, zero-load authoring, backup remapping, move/pure-plan push and detached copy passed. Final APK emulator rerun **1/1 PASS**, with screenshots pulled and inspected. Protocol/localization deterministic checks and `git diff --check` PASS. No physical-device or remote CI result is claimed.
- Intentional limits: applied rule snapshots still change only on the next application; mixed/zero-load prescriptions may require REVIEW. Partial-confirmation push remains its existing v0.14 policy. Private planner input cases, physical-device/full-app end-to-end interaction and remote CI were not run for this correction. No new migration was required; the pre-existing historical migration-suite discrepancy recorded under v0.14.0 was not reclassified as fixed.
...
- Added compact draft/saved-program track settings, record provenance chip and pending hint, scrollable suggestion/settings sheets and semantic calendar blue fills with selection borders. Added 54 Korean/English Android resource pairs through the existing localization authority source/exporter; refreshed localization and current compatibility-consumer inventories through their official generators.
- Backup format 13 / restore schema 12 / PROGRAM_EXECUTION_V1 transports six typed graph row kinds with UUID and immutable workout-source remapping. Legacy format 12/schema 11 remains readable. Source-program-deleted snapshots, current prescription edits, suggestion resolutions and source-ID remapping are covered. Unknown enum/rule versions, invalid quantities, duplicate identities/sequences and dangling or wrong-exercise references fail closed. Move/pure-plan push preserve linkage; generic copy detaches.
- App version `0.5.1.4` / code `501004` unchanged; runtime `RECORD_BASED_PLANNER_0.14.0_KOTLIN_1`. Canonical protocols: PROGRAM-BUILDER-OVERVIEW 3.4.0, OFI-CLASSIFICATION 1.4.0 (presentation only), DATA-BACKUP-RESTORE 1.7.0. Updated existing canonical documents, registry/index, spec and existing release note. Audit commit provenance is finalized by a subsequent docs-only commit.
- Build setup: network-enabled wrapper `--version` first, Android Studio JBR, local `.gradle-user-home`, ANDROID_HOME, `--no-daemon`, quoted `-Pkapt.incremental.apt=false`. Focused tests passed; final `testDebugUnitTest` and `assembleDebug` PASS: **1,415 tests / 247 suites, 0 failures/errors, 3 conditional private-input skips**. The 46 new cases comprise engine 23, persistence/backup 14, layout 6 and calendar 3.
- Then explicitly supplied the user's private CSV to `RealBackupPersonalizedPlannerE2eTest`: restore→generate→save/edit→apply→repository restart→export/restore→cutoff-safety PASS. Re-ran the regenerated compatibility audit in the same invocation: **11 tests passed, 0 skips**. Private CSV content was not committed. The other two conditional cases require the prior training-state comparison phase/artifact and were not rerun for this execution-only change.
- Android17 emulator: final focused `connectedDebugAndroidTest` **2/2 PASS**, including actual 30→31 schema/data preservation and native layout capture. Manually reran capture instrumentation **1/1 PASS**, pulled and inspected eight PNGs at 320/360/411dp × fontScale 1.0/1.3 plus custom settings top/bottom. Short Korean controls remain one line with no glyph clipping or horizontal overflow. Robolectric layout checks rendered line/glyph extents; comparing cached paragraph maximum width caused an initial false positive. Native first-frame capture was synchronized with compositor presentation. Captures remain ignored under `build/progression-verification`.
- Additional full historical migration run: **22 tests, 20 passed, 1 failed, 1 pre-existing skip**. `migrate24To25UsesCanonicalKeysAndPreservesUnresolvedHistoryForReview` fails at line 639: its old expectation is `single_leg_rdl`, while the unchanged exact migration map sends `ex_201f6426` to `dumbbell_single_leg_rdl`. The test, migration source and schemas 24/25 are unchanged from baseline; no baseline APK rerun is claimed. Migration 8→9 is explicitly ignored because its exported v8 schema is absent. These unrelated historical tests were not rewritten to hide their status.
- Localization generation/deterministic checks and audit PASS: zero unexplained English-mode Korean leaks, zero placeholder errors; Python localization **10/10 PASS**. Protocol validation PASS (8 families/34 protocols). `git diff --check` PASS. Scope diff confirms canonical metadata assets, all `analysis/` physiological calculations and legacy ProgramAutoBuilder are unchanged; personalized builder change is four final-output provenance fields only. No app-version bump, release tag, remote CI or physical-device validation is claimed.
- Intentional limits: arbitrary mixed-load structures require per-set manual editing; missing draft targets safely separate into DIRECT mode; rule edits affect the next application, not existing snapshots; old record provenance is not fabricated; SUPERSEDED remains reserved while current invalidation uses STALE. Native component captures and persistence tests do not claim a complete on-device Record→suggestion→accept user journey.
...
## v0.5.0.34 badminton defaults, weekly core chart, and Pallof correction (in progress)

### Phase 0 consumer and authority audit

- Baseline is `6e423e49a0062d6f9d687ad54a890724fde6c845`, exactly the
  expected `v0.5.0.33 / 500033` `origin/main`.
- `BadmintonTrainingMethodSeries.objectiveKeys` already returns all nine
  `BadmintonObjective.entries` in canonical order. Both the recent 7d versus
  28d comparison and weekly objective chart receive `selectedMethodSet` from
  `BadmintonTransferAnalysisContent`.
- The four-objective display is a transient UI-default defect:
  `defaultBadmintonMethodKeys()` sorts totals and calls `.take(4)`, with a
  four-key fallback. The canonical taxonomy and objective calculator are not
  four-valued.
- `BadmintonObjectiveStimulusCalculator` and
  `BadmintonTrainingMethodSeries.totals()` already preserve all nine objective
  identities, including authoritative zero values. The stacked-bar segment
  builders and legend currently drop zero-valued identities after calculation.
- The 7d/28d path currently uses inclusive recent windows of 7 and 28 days and
  normalizes the 28-day total by `value / 28 * 7`. No temporal change is needed.
- `CoreStimulusCalculator` already owns authoritative daily DIRECT and
  INDIRECT values independently of `cumulativeSeries()`. Only
  `CoreStimulusCard/coreStimulusChartSpec` consumes the cumulative projection
  for the main chart and footer.
- `AnalysisChartTemporalPolicy.weekStart()` and `weeklyDomain()` are the
  existing Monday-Sunday grouping and missing-week domain authorities and will
  be reused for the weekly core presentation.
- `badminton_objective_relations.csv` is generated by
  `analysis_cutover_authority.py` through both the focused generator and the
  canonical exporter. It starts with 278 relations. Pallof and Copenhagen each
  retain inherited DECELERATION/FOOTWORK SUPPORTIVE relations; none currently
  has a Pallof ANTI_ROTATION relation, and Copenhagen has no ANTI_ROTATION or
  LUNGE_REACH relation.
- CoreStimulus V1 and Badminton Objective Stimulus V2 arithmetic, coefficients,
  raw history, Room, backup, OFI, readiness, fatigue, connective tissue, and
  ProgramBuilder are outside this correction.

### Implementation commits

#### `6c09745` - show all badminton objectives by default

- Replaced the transient total-ranked top-four default with all nine canonical
  objectives in canonical order. A `v2-all-nine` saveable-state prefix resets
  historical raw top-four state once; selections saved by the new schema still
  preserve an intentional subset.
- Recent and weekly stacked objective models now retain selected zero-valued
  segments. Only badminton objective charts opt into wrapped multi-row legends
  and zero-category preservation; unrelated stacked charts keep their prior
  behavior.
- The inclusive recent 7-day window, inclusive overlapping recent 28-day
  window, and `28-day total / 28 * 7` normalization are numerically unchanged.
- Focused `BadmintonTrainingMethodSeriesTest`,
  `BadmintonObjectiveSelectionStateTest`, and
  `BadmintonTransferColorPaletteTest` passed.

#### `64b5c7f` - show weekly core stimulus

- Added `CoreStimulusWeeklySeries`, which aggregates authoritative daily DIRECT
  and INDIRECT values through `AnalysisChartTemporalPolicy.weekStart()` and
  `weeklyDomain()`.
- Missing weeks are explicit zeros; TOTAL is always DIRECT + INDIRECT. The main
  UI is a weekly `STACKED_AREA` with INDIRECT below DIRECT and TOTAL as the top
  boundary, and its footer reports the latest displayed week.
- `CoreStimulusCalculator`, coefficients, calculation version, cumulative
  compatibility fields, raw replay, and all protected analysis paths were not
  changed.
- Focused core calculator/weekly/chart tests, localization authority check, and
  `:app:compileDebugKotlin` passed.

#### `6498ab7` - approve Pallof anti-rotation objectives

- Added deterministic decision source
  `docs/metadata_authority/badminton_objective_review_decisions_2026-08-14.csv`.
  It approves `band_pallof_press` and `cable_pallof_press` as
  `ANTI_ROTATION / SUPPORTIVE` with provenance
  `USER_APPROVED_BADMINTON_OBJECTIVE_2026_08_14`.
- Generated relation count changed exactly from 278 to 280. The two reviewed
  rows intentionally have no fabricated legacy evidence key; runtime/model
  validation permits that only for the exact user-approved provenance with a
  PASS status and a nonblank reason. All inherited rows still require evidence.
- Both Pallof variants retain existing `DECELERATION / SUPPORTIVE` and
  `FOOTWORK / SUPPORTIVE`. Copenhagen plank (`ex_a8385c4a`) retains those two
  inherited relations and gains neither `ANTI_ROTATION` nor `LUNGE_REACH`.
- The focused generator, canonical exporter, their deterministic checks,
  targeted Python authority test, `CanonicalAnalysisAuthorityTest`,
  `BadmintonObjectiveAuthorityTest`, and
  `BadmintonObjectiveStimulusCalculatorTest` passed.

### Documentation and release closeout

- Protocol audit target: implementation commit `6498ab7`.
- Badminton taxonomy/transfer and strength volume protocols now name the
  current authority, calculator, weekly projection, UI, tests, decision source,
  generated objective asset, and rotation audit.
- Remaining semantic-review debt: the Copenhagen DECELERATION/FOOTWORK mapping
  is inherited from earlier CHANGE_OF_DIRECTION authority and was preserved,
...
## 2026-09-04 - Record-based planner v0.11 exposure representation

- Started from exact latest `origin/main` baseline `517f43d5743daccf431d87af31870a2bcf65bb53` and kept legacy `ProgramAutoBuilder`, global metadata, OFI, readiness, tissue, strength posterior and Objective V2 coefficient/RPE authorities unchanged.
- Replaced binary movement-presence gap detection with explicit `MovementExposureRepresentation` and `BadmintonObjectiveRepresentation` evidence over adjacent cutoff-anchored 28-day windows and four rolling seven-day confidence bins. Comparisons use normalized personal shares plus conservative peer medians; `.25` and `.50` remain centralized engineering outlier rules and do not claim physiological sufficiency or optimal dose.
- Added typed planner activity-domain resolution for resistance, structured badminton drills, athletic-performance drills, generic court sessions and other rows. Plyometric, ankle-SSC and deceleration work no longer inflates resistance working-set baselines, while loaded strength-authority exercises remain resistance where applicable.
- Preserved weighted Objective V2 evidence separately from exact DIRECT exposure. DIRECT drop remains factual evidence, never-direct exposure is an optional non-pressure developmental observation capped at one item, and automatic direct remediation requires both an exact DIRECT relation and a reviewed or legitimate recent prescription.
- Added separate athletic-performance and badminton-drill bout budgets, single resistance/badminton foundational on-ramps, richer gap/decision provenance and backward-compatible decision JSON. No Room migration or Android version change was required.
...
- 2026-09-21 — B1 ledger-backed stimulus-need correctness closeout: grouped one-source-one-task evidence with DIRECT precedence, kept GENERAL/LOW out of task credit, returned UNKNOWN for unavailable or mismatched ledgers while preserving valid-empty ABSENT, separated full final coverage from trace-reconstructed final-reflow distribution, and made task confidence evidence-owned. Direct/supportive prescription-incompatibility reasons are distinct; production planning consumers remain zero and no duplicate pre-final plan or extra DAO history query was added. Focused B1 tests pass 16/16; Stage A rebalancer regression passes 61/61 and ResidualCompletion passes 21/21. Protocol validation passes 9 families/36 protocols and `git diff --check` passes; Gradle `testDebugUnitTest` and `assembleDebug` remain blocked before task execution by the Windows Java loopback error.

## 2026-09-22 - Phase B1 final CI and correctness closeout

- Regenerated the compatibility-only metadata inventory with `tools/generate_metadata_analysis_contract_audit.ps1 -CompatibilityOnly`; the generated CSV/Markdown now include the current TEST references, including `AthleteStimulusNeedTest.kt`. A second generator run produced identical SHA-256 hashes.
- Preserved Stage A candidate-gate ordering and replaced the invalid total `dayProjectionCalls < candidates` assertion with the observation-only `candidateDayProjectionChecks` metric. The cheap-reject fixture records zero candidate projection checks, while the positive fixture records checks and preserves the exact selected action/skeleton.
- Removed mixed-scope top-level fields from `FinalStimulusNeedAuditResult`; full-program coverage is explicit, representative-week reflow evidence is nested under `finalReflowDistribution`, and compact JSON follows the same scope boundary. A real `PostSplitWeeklyReflow` test now attaches its production trace and compares reconstructed pre/post distributions with direct summaries.
- Added Robolectric B1 evidence coverage from `CanonicalExerciseMetadataRepository → StimulusExposureLedger → PlanningHistorySnapshot → StimulusNeedEvidenceIndexBuilder`, including canonical sentinels, generic-court separation, direct-strength response eligibility, and the one-source POWER/RFD multi-view contract.
- Focused direct verification: Stage A `BoundedDayRebalancerTest` 63/63 (cheap fixture: 2 candidates, 0 candidate projection checks, 0 accepted actions; positive fixture: 2 candidates, 1 candidate projection check, 1 accepted action); Stage B `AthleteStimulusNeedTest` 18/18; metadata audit 10/10; metadata generator deterministic; protocol validator 9 families/36 protocols; `git diff --check` passed. The Gradle focused test and `assembleDebug` commands remain blocked before task execution by the Windows Java loopback connection error, so GitHub Actions was not triggered by this unpushed closeout.

## 2026-09-22 - Phase B2 ledger-backed quality-dose shadow history

- Added a transient `historyStart` to the Phase A ledger builder. The actual generation path still performs one history DAO load and one ledger build, but starts the ledger at `min(cutoff - 55 days, oldest completed ISO-week start)` so the reviewed eight completed-week quality baseline is covered for Monday and midweek cutoffs. B1 queries retain their exact cutoff-relative `0..6`, `0..27`, `28..55`, and `0..55` windows.
- Added `LedgerBackedQualityDoseHistoryAnalyzer` as a shadow-only evidence source. It folds the same ledger set observations once per completed ISO week, applies the shared realized-prescription compatibility helper, resolves DIRECT before SUPPORTIVE, counts sessions by `(date, sessionStableKey)` and training days, records incompatible direct/supportive units, keeps court in its separate channel, and preserves the existing successful-dose fallback, quantile, frequency, and confidence mathematics.
- Attached the ledger-backed history and explicit legacy comparison reason codes to the existing `AthleteStimulusNeedProfile` audit payload with compact JSON. `QualityDoseHistory` remains the input to `TrainingDecisionPortfolioEngine` and `TargetStimulusPlanEngine`; no selection, prescription, placement, OFI, tissue, Room/schema, backup or cloud authority was changed. Added horizon, direct/supportive precedence, session/day identity, incompatible prescription, zero/unavailable, median/frequency and canonical integration coverage.
- Focused correction rerun and final CI: run `35670651771` for commit `4e9691ca` passed protocol validation, community/cloud contracts, whitespace, all `1,937` debug unit tests with `0` failures and `4` skips, debug APK assembly, CI signer verification and artifact upload. Local Gradle remains blocked before task execution by the Windows Java loopback error; the unrelated metadata audit still reports the pre-existing `META-SEED-CSV-FALLBACK` configuration gap.

## 2026-09-22 - Phase B3 canonical training decision portfolio shadow

- Added `StimulusTrainingDecisionPortfolioEngine` as an observation-only B3 layer over the existing B1 need decisions and B2 ledger-backed quality baseline summaries. Its quality matrix distinguishes personal direct baseline, valid completed-week history without direct baseline, and unavailable ledger evidence; supportive-only history is never promoted to direct baseline.
- Added separate quality/task strategy models, legacy-priority parity, need/baseline confidence separation, directional task semantics with no numeric task baseline authority, and a per-quality/per-task comparison against the unchanged legacy `TrainingDecisionPortfolio`. The compact B3 payload is attached under `AthleteStimulusNeedProfile.trainingDecisionPortfolioShadow` and does not duplicate B2 weekly evidence.
- Kept `QualityDoseHistory → TrainingDecisionPortfolio → TargetStimulusPlan` as the only target-plan input. No selection, prescription, regional, placement, frequency, OFI, tissue, reflow, production-output, Room/schema, backup or cloud authority changed; B4 target generation remains out of scope. Added focused matrix, confidence, supportive-only, task, comparison and compact-payload tests.
- Pushed implementation and compile correction to `main`: `c6c390d8` followed by `e8de3b95`. GitHub Actions run `35674582154` completed successfully for `e8de3b95`: protocol validation, community/cloud contracts, whitespace, full `testDebugUnitTest`, `assembleDebug`, CI signer verification and `WhatYouGottaDo-debug-apk` artifact upload all passed. The local Windows Gradle invocation remains blocked before task execution by the loopback connection error.

## 2026-09-22 - Phase B3 minimal verification closeout

- Corrected task comparison attribution: a task semantic difference caused by a changed Need now reports `ACTION_CHANGED_BY_CANONICAL_NEED`; a same-Need directional mismatch reports `ACTION_DIRECTIONAL_SEMANTICS_DIFFER`. `ACTION_CHANGED_BY_CANONICAL_BASELINE` remains quality-only because B3 has no canonical task baseline.
- Strengthened the task strategy matrix with exact assertions for every `TrainingNeedDecision`, added task semantic-match and Need-difference regression fixtures, and added real canonical B1 → B2 → B3 coverage using `CanonicalExerciseMetadataRepository`, `StimulusExposureLedgerBuilder`, `AthleteStimulusNeedEngine`, `LedgerBackedQualityDoseHistoryAnalyzer` and `StimulusTrainingDecisionPortfolioEngine`. Production planning behavior and authority flags remain unchanged.

## 2026-09-22 - Phase B4 canonical stimulus target plan shadow

- Added `StimulusTargetPlanEngine` as a separate B4 model family over the summarized B3 portfolio and B2 ledger-backed quality-dose bands. Personal baseline strategies copy the exact weekly Q25/median/Q75 direct-unit and direct-session envelopes; restore carries `PERSONAL_RESTORE_BASELINE`, hold/redistribute carry `PERSONAL_SUCCESSFUL_DOSE`, and no progression or reduction multiplier is introduced.
- Added explicit `StimulusTargetNumericAuthority` states, malformed/unavailable baseline fail-safe handling, reference-only exposure-week bands/frequency, non-additive quality envelopes, and directional/non-numeric task targets. Supportive-only evidence never becomes a direct target and B5 candidate coverage remains out of scope.
- Attached `stimulusTargetPlanShadow` with compact canonical targets, an observation-only comparison against the unchanged legacy target plan, and a current-program audit that reuses `FinalStimulusNeedAuditResult` and normalizes full-program coverage by planning horizon. The legacy target path, generated program, and all planner authorities remain unchanged.

## 2026-09-22 - Phase B5 canonical target-aware selection shadow

- Added `StimulusTargetCandidateSelector` over the canonical B4 `StimulusTargetPlan`. It orders targets deterministically, requires direct quality/task identity, excludes supportive-only and generic-court coverage, reuses multi-target identities, and declines new identity for reduction, redistribution-direction-only, no-minimum and unresolved strategies.
- Added lexicographic candidate ranking using the shared provisional Strength/Hypertrophy exposure classifier, personal history, free-weight compatibility, metadata confidence, redundancy and stable key. B5 records identity and existing-prescription set shape only; it does not authorize target-compatible reps/load/RPE/progression or placement. Existing `PersonalizedPrescriptionPlanner` supplies the probe set count and B6 owns compatibility realization.
- Added an explicit in-memory `generatePreparedStimulusSelectionComparison` that builds unchanged CONTROL and a separate `CANONICAL_SELECTION` pass through the existing material-demand, prescription, capacity, placement, OFI/tissue, completion and reflow machinery. It returns identity differences, B4/current-program and experimental final audits, compact traces and no winner; normal `generatePrepared` remains untouched and no regional experiment is mixed in.
- Added focused selector, comparison, direct-vs-supportive, history-ranking, control-presence, multi-target, fail-safe and compact-payload tests. Protocol documentation and registry are versioned to 3.36.0. Local Gradle remains blocked before task execution by the known Windows Java loopback error; CI is the authoritative validation path.
- CI closeout for `e13c7fbb`: run `35688865728` passed protocol validation, community/cloud contracts, whitespace checks, `testDebugUnitTest`, debug APK assembly, CI signer verification and artifact upload. The uploaded `app-debug.apk` is 64,726,039 bytes with SHA-256 `53D9678BC44140AE3E6386AD25FAE6B347DF9C56D1460FA3063057825095F713`. Local Windows Gradle remains blocked before task execution by the known Java loopback connection error.

## 2026-09-22 - Phase B5 minimal verification closeout

- Added a real Robolectric service-level A/B integration test. It seeds the in-memory Room repository, reads resolved canonical runtime metadata, invokes `PersonalizedProgramPlanningService.generatePreparedStimulusSelectionComparison`, and compares its CONTROL rows and `personalizedProgramFingerprint` with standalone repository `generatePreparedPersonalizedProgram` for the same preflight, cutoff, answers, and request. The fixture requires a non-empty B5 selection and validates target plan, selection plan, audits, experimental rows, authority flags, and no winner.
- Added `StimulusCandidateMaterializationTrace` to the existing in-memory comparison result and compact JSON. It records effective selected/reused identity, B5 selection, final presence, distinct `(week, day)` occurrences, final `setPrescriptions.size` units, existing experimental audit status, and explicit materialized/absent/control-direct/not-requested reasons. The trace is a read-only observation and preserves the original selection trace; no second build, DAO/history scan, audit, persistence, preview route, or regional experiment was added.
- Added focused unit coverage for present, absent, reused, direct-control/no-selection, no-winner, set-unit/occurrence accounting, compact output, and the explicit B6 compatibility-gap separation. B1-B5 ranking, set-count and normal `generatePrepared` semantics remain unchanged.
- Verification is pending the push-triggered CI run. The local Gradle command remains blocked before task execution by the known Windows Java loopback connection error.

## 2026-09-22 - A-B5 post-audit semantic hardening

- Added explicit reviewed-canonical versus unclassified source authority at the Phase A ledger boundary. Confirmed non-court source sets remain one observation each; generic court sessions remain separate. Production supplies the complete canonical repository identity set, including history-only identities.
- Added typed B1 coverage and source-unit counts. Reviewed canonical non-membership can remain a valid zero; unclassified relevant sources produce UNKNOWN or conservative partial evidence. B2 retains incomplete weeks in audit evidence but excludes them from exact numeric quantiles, B3 remains direction-only for partial baselines, and B4 numeric authority requires complete classification coverage.
- Propagated typed evidence basis through B1-B4 and B5. Strength/Hypertrophy use realized-prescription classification; other qualities use capability-proxy semantics; tasks use canonical task relations. B4 audits expose basis and prescription-realization authority, including the proxy-band caveat and B6 gate.
- Hardened B5 semantics: typed probe compatibility is separate from final skeleton materialization; direct identity verification derives from `StimulusSelectedCandidate.coveredTargetIds`; reused identities are checked per target; no-selection verification is nullable. Candidate ranking uses immutable history indexes and preserves the existing lexicographic order without additional DAO queries or production authority.

## 2026-09-22 - Final A–B5 observability and evidence-authority closeout

- Root causes closed: B2 now distinguishes a complete reviewed baseline, partial unclassified history, no eligible classified history, and an unavailable ledger. Empty weeks and excluded weeks remain visible for audit but cannot create valid zero evidence, contaminate completeness, or change aggregate evidence basis.
- B1 now withholds exact current/prior trend ratios when either decision-relevant window is incomplete and uses conservative current-window-only exposure state. B3 separates ledger availability, observed personal direct baseline, baseline observability, and numeric baseline usability; B4 consumes those facts and inherits B3 confidence rather than rebuilding authority from B2.
- Task evidence basis propagates B1 → B3 → B4, and `StimulusSetObservation` requires explicit classification authority. B4 diagnostics distinguish shadow weekly target authority from reference-only exposure-week distribution/frequency diagnostics.
- Added focused `StimulusEvidenceObservabilityTest` coverage for reviewed zero, no history, excluded and incomplete weeks, prior-window uncertainty, task basis propagation, and B3/B4 authority consistency. Production generation and goldens remain frozen; Phase B6 remains deferred.

## 2026-09-22 - Final A–B5 CI verification

- Follow-up correctness fixes were pushed as `dca7bff3` (B4 unavailable-ledger authority guard) and `0c697b50` (exclude cross-file `private` helpers from the record-based architecture dependency audit). The latter removes a false legacy edge caused by the test's package-wide symbol heuristic; it does not alter production planning behavior.
- Final GitHub Actions run [35741545417](https://github.com/Mascollel-Ko/WhatYouGottaDo/actions/runs/35741545417) passed protocol validation, community/cloud contracts, whitespace checks, all 1,976 debug unit tests with 0 failures and 4 skips, debug APK assembly, CI signer verification, and artifact upload.
- Uploaded artifact `WhatYouGottaDo-debug-apk` digest: `sha256:1e8e88fba2a0a077a2326980322b0a18f2372d3285d40bf4c4d0afa4044c28d2`; downloaded `app-debug.apk` SHA-256: `8C19C2BDA193D7CA1F92E649B044ED108C482334758E84CE7BF06313931FED72`.
- The final CI-verified implementation remains observation-only through B5; no Phase B6 prescription realization or production-generation authority was enabled. Local Windows Gradle remains blocked before task execution by the known Java loopback connection error.
## 2026-09-23 — Phase B6 semantic correction audit (protocol 3.40.1)

- Starting main was `51828b73759c5756dad824a019ac349322123a2a`. The correction keeps `StimulusSetObservation.classificationAuthority` as reviewed canonical source authority and carries reviewed realization status separately. Strength/Hypertrophy realization gates no longer control POWER/RFD/SSC/endurance/cardio/mobility capability proxies or canonical task evidence.
- Reviewed gate failures now produce typed `REVIEWED_NON_REALIZATION` zeros; missing authority/input remains `UNCLASSIFIED`. Hypertrophy accepts valid reviewed RPE without an independent 1RM, uses an independent reference plus reviewed curve only when RPE is absent, and rejects unresolved external load. Exact current-session references use prior state; earlier-session fallback uses posterior state with temporal baseline guards.
- B6.1 now evaluates planned compatibility from the existing CONTROL/CANONICAL_SELECTION materialization using exact `(stableKey, selectionRole)` owners. It supports explicit CONTROL-existing provenance and ambiguity statuses, enforces the current exercise-local canonical Strength posterior ≥70% boundary without auto-increasing load, preserves set count, and removes the fabricated `currentRealizations` dependency and extra program build. The Hypertrophy `8 reps / 0 kg` path remains shadow-only.
- Implementation commits: `4c6c4165` (source/realization authority split), `db79c5ce` (planned compatibility and proxy wiring), `a1908053` (typed owner-table imports), and `63a404b8` (materialized B6 probes and service integration assertions), plus `bb325ef0` (reviewed-zero and proxy-uncertainty correction) and `7a8b5e39` (explicit compatibility-helper split). Hosted run `35814042150` on final HEAD `fd4cc95dfb8519ef027484bc20cb416b669ca33a` passed: 1,989 tests completed, 0 failed, 4 skipped; Community/Cloud contracts, protocol validation, whitespace, debug APK assembly, signer verification, and upload all passed. Artifact `10730439892` has uploaded-zip digest `sha256:3a3ee5c681d81bdca2bb4b15159484bd125c70042f07672da23dbde46f3257f7`; downloaded APK SHA-256 is `EA31FF6B74A6D154D2366977E1438E11D6F3389A3768B67504BB40E1203EEBFA`. Local Gradle remains blocked before task execution by the Windows Java loopback error.
## 2026-09-25 — B10.0 pre-change Hypertrophy semantic audit (protocol 3.45.0)

- Starting verified `main` is `bbaf23d65d27c167d6945bc83e7d4ad9ad2a45a1` (protocol `3.44.2`). This entry records the source audit before the B10 implementation change.
- `StimulusExposureLedger` already stores one reviewed realization per canonical set, including exact stableKey/date/set identity, resolved load, RPE/RIR-derived classification, and classification authority. `RealizedStimulusClassifier` classifies reviewed resistance sets with direct quality identity and positive resolved load: Strength is 1–6 reps with an independent 1RM and at least RPE 6 or RIR 4-or-less; Hypertrophy is 7–15 reps with at least RPE 7 or RIR 3-or-less. Missing effort/load is `UNCLASSIFIED`, and below-threshold effort is reviewed non-realization. Hypertrophy can use reviewed RPE without an independent 1RM; no Strength 70%/1–6 rule is reused for Hypertrophy.
- `AthleteStimulusNeed`, `LedgerBackedQualityDoseHistory`, `StimulusTrainingDecisionPortfolio`, `StimulusTargetPlan`, `StimulusTargetCandidateSelector`, and `StimulusExperimentalReadiness` already carry typed Hypertrophy evidence through the same B2–B7 shadow chain. Strength and Hypertrophy use `REALIZED_PRESCRIPTION_CLASSIFIED`; capability-proxy qualities remain separate.
- B4 already exposes `PERSONAL_SUCCESSFUL_DOSE` and `PERSONAL_RESTORE_BASELINE` numeric authority, with `NONE`, `DIRECTION_ONLY`, and `UNRESOLVED` fail-closed states. B5 preserves exact `(stableKey, selectionRole)` ownership. B6.1 already produces typed compatibility/resolution results and the canonical Hypertrophy effort target (RPE 7 / implied RIR 3), but B6.2 currently returns `NO_EXECUTABLE_AUTHORIZATION` for every Hypertrophy target. The Hypertrophy safe proposal currently emits a provisional 8-rep/0-kg shape, and the planned resolver treats provisional zero load as compatible; both are non-executable gaps to close.
- B6 materialization already audits the complete requested horizon with exact owner identity, funded-set multiset/subset validation, weekly shortfall/overrun, and prescription preservation. B8/B9 remain Strength-only (`STRENGTH_V1`) and must continue to reject material Hypertrophy changes to CONTROL. No Room/backup/schema field or additional build is required by the audited model.

## 2026-09-25 — B10.0 implementation/test closeout (protocol 3.45.0)

- Implementation/test commits: `b89f054d` (executable Hypertrophy B6 authority), `beecc511` (restore the existing regional provisional shadow fixture), `b075706d` (proposal-set validation correction), `db147766` (B8 Hypertrophy out-of-scope regression coverage), `55c7087f` (Strength service fixture isolation), `e3095e97` (preserve executable owner authorization targets across multi-quality readiness attribution), `5bd32e12` (B8 material attribution scope), `0936b8dd` and `1261f59c` (Strength-only exact authorization/materialization), and `df90072d` (service assertions scoped to Strength). Final implementation/test SHA: `df90072de50ddfcf8098154eddfac5662e7939db`.
- The B6.1 resolver now requires positive owner-local planned load for Hypertrophy, gates executable resolution on B4 numeric authority, and repairs only reps inside the existing 7–15 band while preserving the exact owner-local set multiset and prescription fields. B6.2 accepts exact existing-compatible and safe-repair Hypertrophy owners using the existing authorization statuses; provisional 8-rep/0-kg, unknown load, unavailable effort/realization evidence, nonnumeric targets, wrong roles and wrong exercises remain non-executable.
- Added pure coverage for reviewed Hypertrophy realization, too-easy and unknown effort, existing-compatible owners, safe repair, missing numeric authority, provisional 8/0 rejection, exact-role/load borrowing rejection, full-horizon materialization, funded multiset integrity, and the B8 `STRENGTH_V1` non-Strength boundary. Existing Strength tests and the real regional provisional shadow fixture remain unchanged.
- Pre-change audit is recorded above. The current production path remains one CONTROL plus one EXPERIMENTAL build, no third build, no schema/backup fields, no HYPERTROPHY_V1 scope, and no B9 routing change.

### B10.0 verification record

- Local `./gradlew :app:compileDebugUnitTestKotlin`: blocked before task execution by the known Windows `java.io.IOException: Unable to establish loopback connection`; hosted CI is the execution authority. Local `git diff --check` and protocol validation pass.
- Hosted implementation CI run [`36158576136`](https://github.com/Mascollel-Ko/WhatYouGottaDo/actions/runs/36158576136) on `df90072de50ddfcf8098154eddfac5662e7939db` passed. The debug unit suite completed **2,053 tests, 0 failed, 4 skipped**; Community/Cloud contract tests, protocol validation (**9 families, 36 protocols**), whitespace checks, debug APK assembly, CI signer verification, and artifact upload all passed.
- Uploaded artifact `WhatYouGottaDo-debug-apk` is artifact ID `10875332149`, size `64,872,859` bytes, with uploaded ZIP digest `sha256:3367fada0ed0a594bc466e1fc92d1529d503787c74f6f8dbc3cbfa55b181a85b`. Downloaded `app-debug.apk` SHA-256: `EC35222F0A6420EDC936CA3BCBAB7B4B689E61FDD9E51E8091A0DECD05C4A1D6`.

## 2026-09-26 — B10.1 multi-quality exact authority closeout (protocol 3.45.1)

- Starting verified `main` was `4fcfeb0ae5c0f0e1a1a6f0615ae34df3048a0b1b`. The implementation is now on `1edd70063770fc2f38dd93b39e9551920f6435a0` and keeps the B10.0 schema, backup, rollout, and B9.2 boundaries unchanged.
- Added a lossless executable prescription authority keyed by `(stableKey, selectionRole, quality)` alongside the B5 owner key `(stableKey, selectionRole)`. Multi-quality rows are arbitrated deterministically: a single authority or semantically identical rows may project to one shared owner prescription; distinct Strength/Hypertrophy prescriptions resolve as `B6_MULTI_QUALITY_OWNER_PRESCRIPTION_CONFLICT`, with no priority or last-write-wins collapse. Quality-specific lookup and slicing remain direct and role/exercise exact, and unresolved owner-only lookup cannot fabricate a prescription.
- Strength remains `FULLY_ENCODED`; executable Hypertrophy remains `CONDITIONAL_ON_UNPERSISTED_EFFORT`. B8/B9 stay Strength-only: non-material Hypertrophy provenance does not poison Strength cutover, while material Hypertrophy attribution blocks with `B8_CUTOVER_V1_NON_STRENGTH_CHANGE_OUT_OF_SCOPE`. No `HYPERTROPHY_V1`, schema/backup migration, or third build was introduced.
- Added pure multi-quality authority tests, mixed B7/B8 provenance tests, and a real Room/service Hypertrophy fixture. The fixture uses an older positive-load 10-rep history plus recent 5-rep sessions to exercise B4 numeric authority, exact B5/B6 owner resolution, a safe Hypertrophy repair and full two-week materialization while remaining shadow-only; B8 routes the material change to CONTROL and B9 preserves CONTROL.

### B10.1 verification record

- Local `./gradlew :app:compileDebugUnitTestKotlin` remains blocked before task execution by the known Windows `java.io.IOException: Unable to establish loopback connection`; hosted CI is the execution authority. `git diff --check` and the protocol documentation validator pass for the final documentation change.
- Hosted implementation CI run [`36230959877`](https://github.com/Mascollel-Ko/WhatYouGottaDo/actions/runs/36230959877) on `1edd70063770fc2f38dd93b39e9551920f6435a0` passed. The debug unit suite completed **2,060 tests, 0 failed, 4 skipped**; Community/Cloud contracts, protocol validation (**9 families, 36 protocols**), whitespace checks, debug APK assembly, CI signer verification, and artifact upload all passed.
- Uploaded artifact `WhatYouGottaDo-debug-apk` is artifact ID `10902274979`, size `64,877,992` bytes, with uploaded ZIP digest `sha256:0998c599beba42684d7330093d04069097c118d28ddf93e918dbd9dff24b0e40`. Downloaded `app-debug.apk` SHA-256: `3EAFB2DA0287303193EA9694A45ACA16300C662C2E73F397D87F1BF9E75BC9B4`.
- Production remains one CONTROL plus one EXPERIMENTAL build with a third-build count of zero; progress/fingerprint routing is unchanged and the B9.2 boundary remains intact.
- Final documentation-HEAD CI run [`36231761718`](https://github.com/Mascollel-Ko/WhatYouGottaDo/actions/runs/36231761718) on `fad034fa79f21813bbd55cf90a32b871d5723f4d` passed the same **2,060 tests, 0 failed, 4 skipped**, 9-family/36-protocol validation, contracts, whitespace, APK assembly and signer checks. Its uploaded artifact is ID `10902289112`, size `64,878,293` bytes, with ZIP digest `sha256:b6b2a03592909a77bafe7650634179f874ab2fd70b0c3ce01415bd63337c9453`.

## 2026-09-26 — B10.2 owner-local multi-quality conflict localization (protocol 3.45.2)

- Starting verified `main` was `7facfe633958445089adc5ea58d44ab2a5fe7d27` (protocol `3.45.1`). Implementation commits are `baaeae6508cac50f9aa1d8ce279f1100008b0c8b` (typed owner-local disposition, planner/capacity/frequency guard, B7 diagnostic handling), `7a3d49cf781a2fb509b4ce0e3e929f64ae92604c` (pure, builder, mixed B8/B9 tests), and `88ed34c4460c5e3195aae3ceff9e393823daec39` (ordinary fallback compatibility correction). The docs update records protocol `3.45.2` against the implementation/test SHA `88ed34c4460c5e3195aae3ceff9e393823daec39`.
- Same-owner Strength/Hypertrophy conflicts retain lossless `(stableKey, selectionRole, quality)` authority rows and diagnostics while exposing typed `EXECUTABLE_EXACT_AUTHORITY`, `PRESERVE_CONTROL_OWNER`, `EXCLUDE_CONFLICTING_ADDITION`, and `NO_EXECUTABLE_AUTHORITY` dispositions. Existing CONTROL owners use the exact CONTROL prescription; new conflicted owners are excluded before placement, capacity tracing and frequency expansion. Conflict audits are `NOT_MATERIALIZED` with zero units and `B6_MULTI_QUALITY_OWNER_PRESCRIPTION_CONFLICT`; no quality priority or arbitration was introduced.
- B7 mixed provenance has no material attribution for the unchanged preserved conflict owner. A valid independent Strength owner remains eligible for B8 `AUTHORIZED_FOR_BOUNDED_CUTOVER` and B9 `B8_STRENGTH_V1`; a conflict-only comparison is B7/B8 `NO_MATERIAL_CHANGE` and routes CONTROL. Strength-only scope, B9.2 typed failure behavior, no `HYPERTROPHY_V1`, no persistence/schema migration and no third build remain unchanged.
- Local Gradle uses the user profile value `JAVA_TOOL_OPTIONS=-Djdk.net.unixdomain.tmpdir=C:\GradleIpc`; `C:\GradleIpc` exists and the Gradle JVM reports the same Unix-domain temporary directory property. Focused builder, materialization, B7/B8/B9 and real Room/service Strength/Hypertrophy fixtures pass with this environment. Full local suite and hosted CI results are recorded below after the final verification run.

### B10.2 verification record

- `git diff --check` passed before the documentation commit. The final local verification includes the focused conflict, materialization, readiness, cutover, builder and Room/service fixture suites; no schema or backup files changed.
- The user-profile Gradle environment was verified as `JAVA_TOOL_OPTIONS=-Djdk.net.unixdomain.tmpdir=C:\\GradleIpc` with `C:\\GradleIpc` present. Focused pure, builder, materialization, B7/B8/B9, and real Room/service Strength/Hypertrophy suites passed. The full local `:app:testDebugUnitTest` reached the test phase but the Windows Robolectric native runtime terminated the executor with `EXCEPTION_ACCESS_VIOLATION` in `robolectric-nativeruntime.dll`; its partial report is not treated as a completed full-suite result.
- Hosted implementation CI run [`36243711720`](https://github.com/Mascollel-Ko/WhatYouGottaDo/actions/runs/36243711720) on `20676643361d0a49702df4d84eba49e47e2aef1f` passed. The debug unit suite completed **2,065 tests, 0 failed, 4 skipped**; Community/Cloud contracts, protocol validation (**9 families, 36 protocols**), whitespace checks, debug APK assembly, CI signer verification, and artifact upload all passed.
- Uploaded artifact `WhatYouGottaDo-debug-apk` is artifact ID `10906018732`, size `64,888,382` bytes, with uploaded ZIP digest `sha256:856dcdafd3933b241c9fb8c2770020d16f4c9d8e4872cb3431ce3f51eb0cc939`. Downloaded `app-debug.apk` SHA-256: `7AA8B64BAF390F113CBEC08A6B750F0D23D6DEF90FBC0F76AE8235F6BA0BAAB7`.

## 2026-09-27 — B11.0 executable effort prescription contract (protocol 3.46.0)

- Starting verified `main` was `f499dc86d0db74b5c6606cf13dd82622ad7377f7` (protocol `3.45.2`). Final implementation and focused-test commit is `cf82d1a6`; the protocol registry and overview intentionally record that implementation/test SHA as `lastAuditedCommit`.
- Added nullable canonical `ProgramSetPrescription.targetRpeMin`, Room schema version 35 with explicit migration 34→35, persisted progression original/planned targets, target-aware program and personalized fingerprints, exact funded-subset semantics, CSV/community/progression wire and backup compatibility, and compact common/mixed planned-RPE display.
- B6 Hypertrophy authorized compatible/safe-repair funded sets now encode minimum RPE `7.0`; only complete valid target encoding is `FULLY_ENCODED`. Provisional `8 reps / 0 kg` remains non-executable. Actual `WorkoutSet.rpe` remains untouched and independent. Strength B8/B9 authority, owner identities/signatures, `HYPERTROPHY_V1`, and third-build boundaries remain unchanged.
- Local verification on the user-profile Gradle environment (`JAVA_TOOL_OPTIONS=-Djdk.net.unixdomain.tmpdir=C:\GradleIpc`): `:app:compileDebugUnitTestKotlin` passed; focused B6 materialization/integration, effort contract, Room migration, persistence/apply/edit/manual-set, and wire tests passed.
- Compatibility closeout preserved null-target legacy wire payloads and pre-B11 fingerprints, added v34→35 migration coverage to historical migration fixtures, and kept the existing 29-persona parity goldens unchanged; the focused compatibility set completed **28 tests, 0 failed**.
- Hosted implementation CI run [`36262306504`](https://github.com/Mascollel-Ko/WhatYouGottaDo/actions/runs/36262306504) on `eb0a65b70526d0f00472475c43a25fae403f8bf1` passed in 10m51s. The debug unit suite completed **2,072 tests, 0 failed**; protocol validation (**9 families, 36 protocols**), Community/Cloud contracts, whitespace checks, debug APK assembly, CI signer verification, and artifact upload all passed.
- Uploaded artifact `WhatYouGottaDo-debug-apk` is artifact ID `10912674225`, size `64,894,569` bytes, with uploaded ZIP digest `sha256:31b1e04bb79d2da322637d845fa5f63250db40c2dec823bc3c221d1cb30c60df`.
## 2026-09-27 — B11.1 executable effort threshold semantic closeout (protocol 3.46.1)

- Starting verified `main` was `354ff2a098985bd232f67c2744cdda9b269f66a9` (protocol `3.46.0`). Implementation and focused-test commit is `032ae7cc`; the protocol docs record that implementation/test SHA as `lastAuditedCommit`.
- Centralized the canonical quality-specific effort mapping and sufficiency check in `StimulusEffortPolicy.kt`. Numeric validation remains in `validatedTargetRpeMin()` (`finite`, `1.0..10.0`); Hypertrophy executable authority additionally requires every funded set to satisfy the canonical minimum RPE `7.0`, inclusive. Null, invalid, `6.9`, and `6.0` are insufficient; `7.0` and `8.0` are sufficient; mixed `[7.0, 7.0, 6.0]` fails as a whole.
- Materialization now derives execution authority from threshold sufficiency while preserving exact prescription matching separately. Preserved RPE 6 remains exact in shape but is not fully executable and reports `B6_EFFORT_TARGET_BELOW_CANONICAL_MINIMUM`; an authorized RPE 7 versus experimental null still reports `B6_EFFORT_TARGET_NOT_PRESERVED`; authorized RPE 7 versus experimental RPE 8 remains exact-preservation failure despite sufficient effort. Normal B6 generation remains RPE 7 and safe repair remains fully encoded.
- No Room/schema/migration/backup/UI/manual-editor/actual-RPE changes were made; Room remains v35, `HYPERTROPHY_V1` is absent, Hypertrophy remains shadow-only, Strength/B8/B9, B9.2 and B10.2 boundaries remain unchanged, and the build count remains one CONTROL plus one EXPERIMENTAL with no third build.
- Local environment was verified as `JAVA_TOOL_OPTIONS=-Djdk.net.unixdomain.tmpdir=C:\GradleIpc` with `C:\GradleIpc` present. `:app:compileDebugUnitTestKotlin` passed. The focused threshold/materialization/regression set passed, including `StimulusEffortThresholdTest`, `StimulusPrescriptionMaterializationTest`, `StimulusSelectionServiceIntegrationTest`, `StimulusMultiQualityConflictLocalizationTest`, `StimulusProductionCutoverAuthorityTest`, `StimulusProductionRoutingTest`, `StimulusProductionFailureBoundaryTest`, and `ProgramEffortContractTest`.
- The full local `:app:testDebugUnitTest` reached the test phase but failed when the Windows Robolectric native runtime terminated the executor with `EXCEPTION_ACCESS_VIOLATION (0xc0000005)` in `robolectric-nativeruntime.dll+0x5c22` under JBR `21.0.11+1-1163.116-jcef`. This is the known JVM/native infrastructure crash; compile and all affected focused tests passed, so it is recorded separately from production regressions.

### B11.1 hosted verification record

- Hosted implementation CI run [`36269562508`](https://github.com/Mascollel-Ko/WhatYouGottaDo/actions/runs/36269562508) on implementation/docs HEAD `684cab2ac7bb3bb0067e4960d3b733a662cc6133` passed protocol validation, Community/Cloud contracts, whitespace checks, the full debug unit-test task, debug APK assembly, CI signer verification and artifact upload. The suite contains **2,078 tests, 0 failures, 0 reported skips** (2,072 on the B11.0 baseline plus the six B11.1 regression tests).
- Uploaded artifact `WhatYouGottaDo-debug-apk` is artifact ID `10915107072`, size `64,897,021` bytes, with uploaded ZIP digest `sha256:6dee8d39c9e3a96cacc09ad91030ee81651ceb0d26977a5364bb287bdf0f85dd`. Downloaded `app-debug.apk` SHA-256: `1EA2702704E876B7B5A998E33A97A8924D82458751B015DFA5148F0E835E2E71`.
